...
>
> Until Emil decides, **no migration runs.** That is not a blocker to be worked around; it is the current state of the system.
...
- **Migrations:** **there is no sanctioned DDL path and the choice is Emil's, pending** (Law 8, corrected 2026-08-26). `~/.config/cedrus/migrate/run-migration.mjs` does not exist on this machine and was never in version control; `supabase/migrations/` does not exist in this repo (only a gitignored `supabase/.temp/`). When a path is chosen: in order, before the code that needs them, and a runner verifies DDL only.

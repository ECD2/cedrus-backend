...
- **The new line reads:** `N email messages are unreviewed (of M eligible; K read), the oldest D days old.` — with `at least N` and `of at least M` when the candidate pool itself filled (`total_is_floor` qualifies BOTH, since both come from the same capped read). **K is the payload length, so it follows `enforceTotalSize` trimming; N and M do not** — the payload shrank, the pool did not. When no pool figure is available (a caller that supplied none, or a failed read) the old computation runs and labels its own narrower scope: `N email messages are unreviewed (of the K read), …`. **`unreviewed_total: null` means "not known" and must never be read as zero.**

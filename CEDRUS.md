...
- **`supabase-js` does NOT throw on a database error.** It resolves `{ data, error }`. Verified 2026-07-26: **45 of 101 `supabase.from()` call sites in `src/` never bind `error`.** (**Denominator corrected 2026-08-18:** there are now **114** call sites, so the 101 is stale and the unbound fraction has NOT been re-counted since. Treat 45/101 as the last real measurement, not as current. Re-count before citing a ratio.) Each is a place where a DB failure yields no exception, no log, and a plausible return value.

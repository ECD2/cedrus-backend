...
**Law 8. Migrations run through a sanctioned path, never by hand.** **Anything touching existing DATA shows the plan and waits for Emil.** A runner verifies DDL objects only — anything else needs real assertions written by hand (see II.5, Tooling).

> **DECIDED 2026-08-30 — option 2. THE SANCTIONED DDL PATH IS THE SUPABASE CLI:** one file per migration in `supabase/migrations/`, applied with `supabase migration up --linked`, **never a bare `db push`** (which diffs the schema and applies whatever it thinks is missing). `supabase/migrations/` now exists in this repo and holds one file. The remote history carries six file-less versions that must be reconciled with `supabase migration repair --status applied <version>` before the first apply — the exact commands, and why `migration up` is safe once the history is clean, are in `docs/MIGRATION_PATH.md`. **The paragraph below is the superseded pending-decision note, kept because the four faults it records must not be re-introduced by any future runner.**
>
> **THERE WAS NO SANCTIONED DDL PATH BETWEEN 2026-08-26 AND 2026-08-30.** (Recorded 2026-08-26, Law 12; closed 2026-08-30.) This law read `node ~/.config/cedrus/migrate/run-migration.mjs <ddl.sql>` as though that file existed. It does not: `~/.config/cedrus/migrate/` is **absent from this machine**, the runner was **never in version control** in any of the four clones, and it went with the old machine. `supabase/migrations/` **does not exist in `cedrus-backend`** either — only a gitignored `supabase/.temp/`. So a session obeying this law literally finds nothing and cannot tell "absent" from "I looked in the wrong place."
>
> The two options, from the 2026-08-17 record, both still open:
>
> 1. **Rewrite a runner** using `pg` + `DATABASE_URL`, reproducing the old contract (pre-check → BEGIN → apply → in-transaction asserts including a control → COMMIT/ROLLBACK → fresh post-check).
> 2. **Supabase CLI**, `supabase migration up --linked`, **per file** — **never a bare `db push`**, which diffs and applies whatever it thinks is missing.
>
> Whichever is chosen, **every new table must enable row-level security in the same migration** — the publishable key is public and ships in the client bundle, so RLS does one hundred percent of the security work.
>
> ~~Until Emil decides, **no migration runs.**~~ **Closed 2026-08-30:** option 2 is the path. A session still never APPLIES one — Law 5 — it prepares the file and prints the commands.
...
### 19. A guard can be live, load-bearing, and still unfalsifiable

**Incident (2026-08-18).** A dead-guard audit mutated every condition enforced
in more than one place. Four mutations to the card rail's compare-and-set left
the entire battery GREEN: deleting `.eq('status', fromStatus)` from
`transitionCard`, making it always return `true`, and making `cardSender` and
`cardFollowup` each ignore a refused claim.

The day before, an uncatchable mutation had meant the guard was DEAD — the
`briefMode` precedence was encoded three times over and no single break was
observable, so the fix was to collapse it into one place. Applying that
conclusion here would have deleted the only thing stopping two overlapping ticks
from sending the same card twice.

The CAS was not dead. Every existing card test drove the happy path, so nothing
ever observed a claim being REFUSED. It was **live but unfalsified**.

**Rule.** An uncatchable mutation has two possible meanings and they demand
opposite actions: the guard is unreachable (collapse it), or the guard is real
and the test never exercised its failing branch (falsify it). Decide which by
asking *what would have to happen in production for this branch to run* — if
that scenario is real, the guard is real. Never delete on the strength of a
green mutation alone.

**Corollary, and it bit inside this same audit.** The first version of the new
CAS test passed for the wrong reason: `seedBase()` calls `__resetCards()`, which
sets `config.briefDryRun = true` (`prelude-cards.js:47`), so the dry-run branch
— not the refused claim — was stopping the send, and the mutation stayed green.
...
- **Bundle numbers in use: 17 (model-timestamps), 18 (interests), 19 (goals), 20 (§6 suppression read), 21 (quota reads), 22 (crisis vs pre-model short-circuits), 23 (relationships writes), 24 (memory silent failures), 25 (consent audit trail), 26 (trial downgrade), 27 (entitlements), 28 (budget guard), 29 (budget vs pipeline ordering), 30 (card rail state machine), 31 (card failure honesty), 32 (card replies vs pipeline ordering), 33 (admin broadcasts), 34 (web onboarding answers), 35 (contracts on POST /api/goals), 36 (inbound SMS allow-list), 37 (outbound SMS allow-list), 38 (CoS daily brief), 39 (logger.scrub: timestamps + UUIDs vs phones), 40 (verify-brief-run: post-deploy log verification), 41 (multi-user isolation), 42 (provisionUser atomicity) — 28–34 merged to local `main` 2026-07-29 morning** (they were branch-only until then). Next free: **49.** (**Bundles 46, 47 and 48 claimed 2026-09-09**, merged to local `main` at the 2026-09-10 morning ceremony: 46 by `test/person-settings.test.mjs` — the brief's identity from `user_settings` rows, P1.4 — with `test/mutate-bundle-46.sh` (13 guards at the merged commit `4d93856`, 15 at the withheld `28208a2`, 0 missed); 47 by `test/auth-api.test.mjs` — A3.1/A3.2 — with `test/mutate-bundle-47.sh` (14 guards, 0 missed); 48 by `test/isolation-proofs.test.mjs` — A2/A5/A10 — with `test/mutate-bundle-48.sh` (13 guards, 0 missed). **45 was never claimed and is free**, but the convention is the number above the highest in use, and 48 is the highest at `1ba6ad0` by `grep -rhoE 'Bundle [0-9]+' test/*.sh | sort -uV`. **Bundle 40 was EXTENDED on 2026-09-09** — the verifier reports an aborted run, the owner id source and the programs block, 67 → 123 checks — and did not take a new number; `test/mutate-bundle-40.sh` still proves its 14 guards, and the five new guards N5 mutated by hand are listed in `docs/SESSION_N5_2026-09-09.md` for the harness, owed and not wired.) (**Bundle 44 claimed 2026-09-09** by `test/normalize-account.test.mjs`, registered in `test/run-all.sh` with `test/mutate-bundle-44.sh` — 21 guards, 0 missed; it applies all FOUR migration files on PGlite and SPAWNS the real Python backfill script against that database through an in-process fake of the Management API endpoint (`SUPABASE_MGMT_API_BASE`), so the dry run is proven to send reads only and `--commit` is proven to produce the shape. 44 was verified free by `grep -rhoE 'Bundle [0-9]+' test/*.sh | sort -uV` BEFORE it was claimed.) (**Bundle 43 claimed 2026-09-09** by `test/programs-foundation.test.mjs`, registered in `test/run-all.sh` with `test/mutate-bundle-43.sh` — 15 guards, 0 missed; it applies all three migration files on PGlite and proves isolation on the signed-in path AND the table-owner path, each with a control. 43 was verified free by `grep -rhoE 'Bundle [0-9]+' test/*.sh | sort -uV` BEFORE it was claimed.) (**Bundle 42 claimed 2026-09-09** by `test/provision-user.test.mjs`, registered in `test/run-all.sh` with `test/mutate-bundle-42.sh` — 8 guards, 0 missed; it runs the REAL migration files on PGlite, an in-process Postgres 17 added as a devDependency, so it is the first stage in the battery that exercises SQL rather than a fake. 42 was verified free by the mandated `grep -rhoE 'Bundle [0-9]+' test/*.sh | sort -u` BEFORE it was claimed.) (**Bundle 41 claimed 2026-08-30** by `test/multiuser-isolation.test.mjs`, registered in `test/run-all.sh` with `test/mutate-bundle-41.sh` — 9 guards, 0 missed. 41 was verified free by the mandated `grep -rhoE 'Bundle [0-9]+' test/*.sh | sort -u` BEFORE it was claimed.) (**Corrected 2026-08-26, Law 12:** this read "Next free: 40" until Bundle 40 was claimed the same night by `test/verify-brief-run.test.mjs`, registered in `test/run-all.sh` with its own `test/mutate-bundle-40.sh` — 14 guards, 0 missed. The number was verified free by `grep -rhoE 'Bundle [0-9]+' test/*.sh | sort -u` BEFORE it was claimed, which is the command this bullet mandates and which this line has itself gone stale against twice.) (**Corrected 2026-08-17, Law 12:** this line read "Next free: 35" while 35, 36 and 37 were already registered in `run-all.sh` — the note went stale when those three merged and nobody moved it, which is the exact failure it warns about. 38 was taken by the CoS daily brief the same day.) **Bundle 38 was EXTENDED on 2026-08-26** — honest email counts, the send-ledger pre-check, and compose/send timing — and did NOT take a new number: it is the same job, the same suite (`test/cos-daily-brief.test.mjs`) and the same harness. `test/mutate-bundle-38.sh` now proves **52** guards (**52 as of 2026-09-04, Law 12:** four owner-source-label mutations were added that day and `src/services/cos/writer.js` joined the harness's restore-checksum snapshot; the count is the harness's own RESULT line, `52 guards proven live, 0 missed`. It read 48 before that) (**corrected 2026-08-30, Law 12: this said 43, and it was already stale before that session touched anything** — the committed harness at `ea82371` contains 47 `mutate` and 1 `mutate2` call, counted with `grep -c '^mutate ' test/mutate-bundle-38.sh`. No mutation was added on 2026-08-30; one was UPDATED, because its `alreadySentToday` pattern no longer matched after the ledger became per-user, and a mutation that does not apply reports ERROR rather than proving anything) (was 36) and gained a `mutate2()` for guards that only break when TWO files disagree — the reader/composer `'unreviewed'` spelling is one such guard, and mutating either file alone leaves them agreeing and the suite green, which would read as "no guard here" when the agreement IS the guard. `src/services/cos/reader.js` was added to that script's restore-checksum snapshot. Station docs have claimed already-taken numbers more than once — and so has this line. **Check every rig, not just `test/run-tests.sh`:** bundles 35–38 are registered in `test/run-all.sh`, not in the concat rig, so grepping only `run-tests.sh` returns 34 and reads as "35 is free." The reliable command is `grep -rhoE 'Bundle [0-9]+' test/*.sh | sort -u`.
...
- **`test/no-nul-bytes.sh` scans `git ls-files` — TRACKED files only. A NEW, UNTRACKED file is invisible to it.** Learned the hard way 2026-08-30: a raw NUL in `docs/SINGLETON_AUDIT_2026-08-30.md` passed every branch run while the file was untracked, then turned the merged-main battery red the moment the commit made it real. **Run the gate AFTER `git add`, never before**, or a new file's first scan is the merge gate. This is also the cleanest demonstration of Law 4 the project has: the pre-merge battery was green and proved nothing about after the merge. (It is the THIRD raw-NUL incident here, and the second where the NUL was typed while writing *about* NULs.)
...
**Multi-user — the allowlist model (established 2026-08-30, Session A)**
- **Cedrus is allowlist-only multi-user.** A small, known set of people, each fully isolated. Governing decision: `CEDRUS_MULTIUSER_AMENDMENT_2026-08-30.md`. **This is NOT a revival of the archived consumer product** — no signup, no tier, no plan, no trial, no billing, no quota, no affiliate surface, and the twelve `archive/*-2026-08-15` tags stay archived. **A session that finds itself adding a `plan` gate has misread it.**
- **The second invariant, and it is absolute: `a user change must never leak a record.`** Sibling to "a channel change must never create a second agent."
- **THE RULE: env vars hold secrets and GLOBAL ARMING SWITCHES only.** Everything that differs per person is a column. `COS_BRIEF_TO`, `COS_BRIEF_USAGE_USER_ID`, `COS_USER_ID` and the hard-coded 11:00 brief hour are each a per-person value living in the environment; `user_settings` replaces all four. If configuration is a variable, the fifth user is a deploy instead of a click.
- **Service role bypasses RLS, so RLS is NOT the isolation boundary on the CoS path.** `src/services/cos/client.js` connects with `COS_SERVICE_ROLE_KEY`. The boundary is `forUser(userId).select(table, build)`, which applies `.eq('user_id', userId)` BEFORE the caller's narrowing; PostgREST builders are additive, so `build` can narrow and cannot widen. **`cosSelect` no longer exists** — an unscoped read is not discouraged, it is unexpressible. The one legitimate cross-user read goes through `cosSelectAcrossAllUsers`, which demands a stated `reason` and logs `cos.read.cross_user` every time. (**Corrected 2026-09-10, Law 12, from `docs/SCOPE_AUDIT_2026-09-09.md` F1-05:** the verb has exactly ONE caller in `src/`, `writer.js`'s bootstrap owner derivation, and THAT is the read this sentence means. `client.js`'s own header names "the admin panel's user list" as the verb's purpose, but that list is `adminOps.js`'s read of the CEDRUS database through the plain service-role client — it cannot go through the CoS verb and does not. The derive path is still present on `main` at `1ba6ad0`; P1.4's withheld second commit `28208a2` deletes it, after which the verb has no caller at all until someone adds one deliberately.) Bundle 41 pins the module's entire export surface, so any new door fails the battery until someone adds it deliberately.
- **The reader was unscoped until 2026-08-30 and its own header said why.** All eight functions in `src/services/cos/reader.js` issued zero `.eq('user_id', …)`, justified by a comment reading "CoS is a one-owner app ... service_role sees every row and there is one owner's worth of rows." True of one person; with two it returns the other person's workstreams, decisions and **email excerpts** (`plain_text_excerpt` — real message bodies, which is what makes it a privacy breach rather than merely wrong data). The comment is rewritten, not deleted: left standing it would talk a future session out of the fix.
- **The send ledger is keyed per USER per day:** `cos_brief_send:<user_id>:<YYYY-MM-DD>`. It was `cos_brief_send:<date>` — one claim for everyone. The second person's brief was refused `already_sent` down the **fail-closed** path, which in the logs is byte-identical to correct duplicate prevention: dad's brief would never arrive and nothing would say so. `ledgerKey()` now THROWS without a user id rather than defaulting, because a default recreates the global key under a new name. The key survives `logger.scrub()` intact — a UUID and an ISO date are both masked ahead of the phone pass (Bundle 39), verified 2026-08-30.
- **`resolveCosUserId()` memoized ONE owner id in a process-global slot** (`let cachedUserId`). Not named in the amendment; found by reading the file. Iterating users in one process, user A's id would be returned for user B and **B's brief written back to CoS attributed to A**. Now a `Map` keyed by CoS project, and unreachable at all once a caller names a user. (**Superseded on the settings path 2026-09-10** — P1.4 commit 1, `4d93856`, merged `a81871c`: `resolveCosUserId({ personId })` reads the person's `user_settings` row through `src/services/cos/personSettings.js`, which holds NO module state and no cache of any kind — Bundle 46 resolves A then B in one process and edits the row between two calls for A. The `Map` survives only on the derive path; the withheld `28208a2` removes that path, the `Map`, `resetCosUserId()` and the writer's `cosSelectAcrossAllUsers` import, after which there is no owner slot of any kind in `writer.js`.)
- **`app_users` gains `role` ('admin'|'member') and `account_status` ('active'|'suspended') as SEPARATE columns.** Collapsing them means revoking an admin silently demotes them and re-activating silently restores admin. `is_app_admin()` answers false for a suspended admin, deliberately.
- **Capability vocabulary is CLOSED, by check constraint: `run_agents`, `write_workspace`, `receive_brief`, `receive_sms`.** One row per user per capability in `user_capabilities`. An absent row and `granted = false` mean the same thing — refused. The constraint means a typo is rejected at write time rather than granting nothing silently.
- **`admin_audit` is append-only by TRIGGER, not by grant.** Service role bypasses grants and RLS both, and it is the only writer this table will ever have, so a permission the privileged path can ignore is not a guarantee. UPDATE and DELETE raise `restrict_violation`.
- **The bootstrap admin (2026-08-31) — one hand-made promotion, and it is not a precedent.** `20260830120000_multiuser_foundation.sql` deliberately grants nobody admin: `role` defaults to `'member'`, and the only `'admin'` literals in the file are the CHECK vocabulary and the `is_app_admin()` predicate — there is no `INSERT` and no `UPDATE ... SET role` anywhere in it (verified 2026-09-03). That is the right default, because a migration that made whoever happened to be in the table an administrator would be a privilege grant nobody reviewed. But no account can be provisioned until one account is an admin, so **exactly one promotion is done by hand, once**: `c6cf9fb9` on 2026-08-31, in the SQL editor, verified by a returning `SELECT`. **This is a bootstrap, not a precedent.** It does not license hand-editing accounts afterwards: once the provisioning path exists, every account — that one included — takes its shape from the same code path, and every role change is an audited admin action in `admin_audit`. **A session writing an `UPDATE` against `app_users` outside the provisioning verbs has misread this.** Two things stated plainly rather than implied: the promotion itself is **Emil's report, recorded here, not verified from this machine** — `c6cf9fb9` appears nowhere in the repo except as the truncated log label two bullets below, and no session here could read prod when this was written (**superseded 2026-09-09 for metadata, see the 2026-09-09 fold at the end of this section:** the read-only Management-API SQL path that `scripts/verify-multiuser-state.py` uses reads prod from a session, and N3 and R6.2a both used it) (the `supabase` CLI is not installed). And **the provisioning path exists as code AND IS APPLIED** (corrected 2026-09-09 evening, Law 12, from a read-only Management API query of prod by the R6.2a session: `supabase_migrations.schema_migrations` carries `20260909120000` and `pg_proc` has `provision_user`; earlier the same day this read "NOT applied", and from 2026-09-03 "does not exist yet"): `provision_user()` is a Postgres function in `supabase/migrations/20260909120000_provision_user.sql`, SECURITY DEFINER, `service_role`-only, doing the sign-in identity, the `app_users` row, the `user_settings` defaults, the `user_capabilities` grants and exactly one `admin_audit` INSERT in one transaction, with `src/services/provisioning.js` as its thin caller and Bundle 42 as its proof (`docs/PROVISION_USER_2026-09-09.md`). The function is live, and the code that calls it is pushed — `origin/main` was at `6fecb69` on the evening of 2026-09-09, which under Law 6 means deployed, though a Railway-log confirmation is still owed (Law 10); until P1.3 runs Emil's own row through it, the audited-code-path guarantee is still the standard this bullet sets rather than a mechanism. **The same read found `user_settings` EMPTY (0 rows) — no row for `c6cf9fb9`** — so anything that resolves per-person configuration from rows (the brief's CoS owner, the today's-program owner) falls back to its env variable for him until P1.3 backfills his account. The function never creates an admin and never records SMS consent — both are separate acts.
- **Production's `auth` schema carries a trigger this repo does not contain** (recorded 2026-09-09 from the P1.2 report). `src/routes/api/auth.js:17` names it — `link_or_create_app_user_from_auth`, firing when an `auth.users` row is created at web signup and linking-or-creating the `app_users` row — and its body appears nowhere in this repo or the sibling repos, and could not be read from this machine. Two consequences. First, **any code that creates a sign-in identity must expect that trigger to act on `app_users` in the same transaction.** `provision_user()` writes the `app_users` row BEFORE the identity so the trigger finds a row to link, then asserts exactly one `app_users` row carries the phone and exactly one carries the new `auth_user_id`; if the trigger created a second account the call raises `integrity_constraint_violation` and the whole account unwinds (Bundle 42 proves both the linking and the hostile create-a-second-account variant). Second, **the trigger's real body is an open question for the first live provisioning (P1.3)** — the migration's CONTROL 1 runs the function against production inside the apply transaction, so a trigger that misbehaves fails the apply rather than a person.
- **The SMS path records consent only when it CREATES the row, so a provisioned person's first inbound text records no consent** (recorded 2026-09-09 from the P1.2 report; **owed in Phase 7, before Dad's first message**). `findOrCreateByPhone` (`src/services/users.js`) sets `sms_consent_at` and `consent_source = 'first_message'` in its INSERT branch only; a row that already exists is returned untouched. `provision_user()` deliberately leaves `sms_consent_at` NULL — an admin cannot consent on someone's behalf (audit finding 13 separates the allowlist step from the compliance step) — so after P1.3 every provisioned account reaches the SMS pipeline as an existing row with no consent, and the first text neither writes `sms_consent_at` nor a `consent_events` row. Nothing currently refuses to reply to such a row. The fix belongs to the inbound SMS gate, not to provisioning: on first inbound from a row with `sms_consent_at IS NULL`, record consent the way `webOnboarding.js` does (`sms_consent_at` + `consent_source` + a `consent_events` row) before any product logic runs.
- **`account_status` fails closed on a PRESENT-but-unrecognised value and OPEN on an ABSENT one.** The absent case is the pre-migration state: failing closed there would lock every user out of the whole API if the code ever reached production ahead of the migration, while failing open reproduces exactly today's behaviour, in which nobody is suspended. Scoped strictly to "the column does not exist yet" — `'banana'` is refused.
- **`email_ai_analyses` is read by the brief but was NOT in the 2026-08-26 live column dump** (that dump covers `agent_runs`, `captures`, `decisions`, `email_messages`, `email_sources`, `open_loops`, `today_briefs`, `workstreams`). **Whether it carries `user_id` is UNVERIFIED.** It matters: `.eq('user_id', …)` against a table lacking the column returns PostgREST `42703`, and the reader fails closed, so the whole brief aborts. Verify before the first live run after deploy.
...
- **`supabase-js` does NOT throw on a database error.** It resolves `{ data, error }`. Verified 2026-07-26: **45 of 101 `supabase.from()` call sites in `src/` never bind `error`.** (**Denominator corrected 2026-08-18:** there are now **114** call sites, so the 101 is stale and the unbound fraction has NOT been re-counted since.) **RE-COUNTED 2026-08-26 (Law 12): 68 of 114 call sites do not bind `error` on the calling line.** The 45/101 figure is retired. **The commands, recorded here so the number is reproducible rather than trusted** — the previous count's method was never written down, which is why it could not be checked and why it sat stale for eight days:
```sh
# denominator — every supabase.from() call site in src/          → 114
grep -rn 'supabase\.from(' src --include='*.js' | wc -l
# numerator — those whose calling line does NOT destructure error → 68
grep -rn 'supabase\.from(' src --include='*.js' | grep -vE '\{[^}]*\berror\b' | wc -l
# bound, for the arithmetic check (68 + 46 = 114)               → 46
grep -rn 'supabase\.from(' src --include='*.js' | grep -cE '\{[^}]*\berror\b'
```
**Read the number honestly, in three parts.** (a) The count is *per calling line*: every binding in this codebase sits on the same line as its `supabase.from(`, verified by inspecting all 114 prefixes, so a line-scoped grep is sound here — but it would silently undercount the moment someone splits a destructure across lines, and it is a textual test, not a dataflow one. (b) **28 of the 68 are a bare `await supabase.from(…)` with no destructuring at all** — those cannot bind an error by construction and are the genuinely blind ones. (c) **3 of the 68 are deferred query builders** (`let q = supabase.from(…)`) whose `error` may well be bound at the later `await`; counting them as unbound OVERSTATES the problem by 3, and they are left in so the number stays conservative and the command stays one line. **Where they live** (`… | cut -d: -f1 | sort | uniq -c | sort -rn`): `people.js` 14, `briefEmail.js` 11, `users.js` 9, `adminOps.js` 8, `relationships.js` 6, `messages.js` 5, `routes/admin.js` 5, `briefs.js` 4, `reminders.js` 2, and one each in `usage.js`, `cards.js`, `broadcasts.js`, `sweeps/clarificationExpiry.js`. That matches the standing note that the remainder is "mostly `people.js` and `users.js`" — though `briefEmail.js` at 11 is the second-largest concentration and has never been named in this register. Each is a place where a DB failure yields no exception, no log, and a plausible return value.
...
- **Nothing in the proactive layer sends SMS today.** `BRIEF_DRY_RUN=true` on Railway, and **ALL FIVE** outbound paths honour it: `weeklyBrief.js`, `dailySweeps.js`, `reminders.js`, `cardSender.js`, `cardFollowup.js`. (**Corrected 2026-08-18, Law 12:** this said THREE and named line numbers that have since moved; the card rail added two more when it shipped.) The only real outbound SMS is the synchronous TwiML reply to an inbound message. Arming the proactive layer therefore takes TWO independent switches — populate `is_core_five` (decides what content is selectable) AND set `BRIEF_DRY_RUN=false` (decides whether anything is sent). Neither alone puts a message on the wire. Note briefs still make their OpenAI call under dry-run.
...
- **The `run-all.sh` false-pass trap — measured 2026-07-26.** The script prints **"ALL WS-B SUITES PASSED" roughly HALFWAY through, with many suites still to run.** Re-measured 2026-08-18: line **1803 of 3588 (50%)**, with **19** suite banners after it. (The 2026-07-26 figures were 1096 of 2404 and thirteen suites; both moved as bundles were added, which is exactly why the position must never be memorised — only the exit code is stable.) `set -e` protects the exit code, so the gate itself is sound. But the banner is a mid-run status line for one workstream, not a verdict. **Gate on `echo $?` — never on that line, and never on eyeballing the tail.** A session that greps for "PASSED" and stops reading will report a green battery it never observed. There is a second banner, "ALL BATTERY SUITES PASSED", at the true end; even that is weaker proof than the exit code.
...
**Repo integrity, logging, and dry-run honesty** (established 2026-08-18)

- **The battery's FIRST stage is now a NUL-byte scan of every tracked file** (`test/no-nul-bytes.sh`). A literal NUL makes git classify a text file as binary: `git diff` prints `Bin N -> M bytes` with no content, so the file silently leaves code review. It happened twice on 2026-08-17, the second time inside the sentence warning about the first. The check verifies its OWN detector on every run — a synthetic file with a NUL must be flagged, a synthetic clean file must not — so it cannot degrade into a check that passes having examined nothing. **492 tracked files, of which exactly THREE are legitimately binary, and the allowlist holds exactly those three** — `web/public/v8/pickleball.png`, `web/public/v8/travel.png`, `web/public/v8/triathlon.png`, the program artwork vendored with the V9 interface on 2026-08-28 (`72d10f0`). So **489 files are scanned**. (**Corrected 2026-09-03, Law 12:** this read *"410 tracked files, zero legitimately binary, so the allowlist is empty and should stay that way."* All three claims were false — the count had drifted by 82 files, the three PNGs were committed five days after this line was written, and the script's `ALLOWLIST` was populated in the same commit. The instruction to keep the allowlist empty was the damaging half: a session obeying it would have deleted three entries and turned the battery's first stage red on three files that are supposed to be binary. Re-count with `git ls-files | wc -l` and settle binariness with `git diff --numstat <empty-tree> HEAD | awk '$1=="-"'` rather than trusting this line — it has gone stale once already.) **The allowlist is an exact-path list and must stay one; never widen it to a pattern**, and every addition is a reviewable exemption that says why the file is binary.
- **`logger.scrub()` used to rewrite any ISO date as a phone number.** The phone regex treats `-` as an internal separator, so `2026-08-15` parsed as one eight-digit number and logged as `[phone:0815]`. Fixed 2026-08-18 by masking ISO timestamps before the phone pass and restoring them after. **NOT by a negative lookbehind:** `(?<![\d-])` also refuses to redact `call-7869727469`, and a false negative there leaks a phone number while the bug it fixes only mangles a date. Bundle 39 pins that case. **Extended 2026-08-20 to mask UUIDs too**, after prod logged `correlation_id="7998d9b5-cc[phone:4996]d1d-…"` — the same hyphen-as-separator fault, eating the field that ties a job's log lines together. Masking is ONE COMBINED PASS over `UUID|ISO`, because the stash restores in document order and two sequential passes would reorder it; any new shape goes into that alternation, never a second pass. **The catch-all phone pattern stays** — field-based scrubbing would let a number in an unlisted field ship in the clear, and a leaked number is worse than a corrupted line. **The corruption is lossy:** a mangled id cannot be recovered from the log, which is why two of the three test correlation_ids are reconstructions rather than the true originals.
- **`logger.event()` SILENTLY DISCARDS any field not in `STRUCTURAL_FIELDS`** (`src/utils/logger.js`), and this is repo-wide, not a CoS quirk. The allowlist is the documented "structured-first, reject-disallowed-keys" rule and it is correct — but a caller passing an unlisted key gets no warning, no throw and no line in the log, so the field simply never existed. Found 2026-08-26 when `cos.compose.ok` was written with `tokens` and `model` and emitted neither. `tokens` and `model` were added to the allowlist (both low-cardinality, non-free-form, exactly the stated criterion). ~~**Others in `cosDailyBrief.js` are still being dropped today and were left alone deliberately as out-of-scope:** `considered`, `eligible_total`, `total_is_floor` on `cos.email.truncated`, and `priorities` / `cited_records` / `subject_chars` on `cos.brief.dry_run` and `cos.brief.writeback_only`.~~ **FIXED 2026-08-26 (Law 12), all six added to the allowlist** (`src/utils/logger.js:60`ff). Each is a number or a boolean, which is the file's own stated criterion and the ONLY reason they qualify — **the allowlist was NOT widened to anything free-form**, and the comment in the source now says so explicitly so the next addition has a written test to meet rather than a precedent to point at. **The allowlist is now falsifiable.** `droppedFields(fields)` (exported, pure) returns the names an object would lose to it; `buildLogRecord()` calls it only when a test has armed the recorder (`armDropRecorder()` / `dropRecords()` / `dropRecordsSeen()` / `disarmDropRecorder()`), so production output is byte-identical armed or not and production pays one falsy check per event. Bundle 38 arms it for the whole suite and asserts that **no** `logger.event()` call made by the CoS job, composer, ledger, writer, client or Resend transport across every fixture loses a field — with three controls, because "nothing was dropped" is an assertion that nothing happened: an unknown field IS reported, an allow-listed field on the same call is NOT, and `dropRecordsSeen()` proves the recorder was pointed at ~71 real events rather than at nothing. **A trap found while writing it, and it is the general lesson here:** three of the six ride on `cos.email.truncated`, and nothing in the bundle was making the real job emit that event — the truncation section drives the composer directly and never reaches `logger.event()`. The aggregate assertion therefore looked like coverage while covering half the change, and a mutation removing `considered` would have stayed GREEN. **An aggregate over emitted events only covers the events something actually emits; enumerate which ones fire before trusting it.** Bundle 38 now drives the real job through the truncation path and the dry-run path and asserts the fields on the real records. `test/mutate-bundle-38.sh` gained guard 14 (four mutations: two allowlist removals, the detector stubbed empty, the seen-counter frozen) and `src/utils/logger.js` joined its restore-checksum snapshot. **Before adding a field to any `logger.event()` call, check the allowlist**, and assert the field's presence in a test rather than assuming it survived.
...
- **The brief's unreviewed count described its own SLICE and called it the inbox** (fixed 2026-08-26, Bundle 38). `computeWorkspaceState` counted `input.email_messages`, which is the selected 20 — so with 25 unreviewed messages it announced "20 email messages are unreviewed" and read the oldest age off those 20. The second number was wrong *structurally*, not by luck: the ranker sorts recency DESCENDING inside a tier, so **the oldest unreviewed message is the row likeliest to have been cut.** The count stopped growing exactly when the backlog did, and the age shrank as the backlog aged. `selectEmailMessages()` now also returns `unreviewedTotal` and `oldestUnreviewedReceivedAt`, both computed over the full eligible pool **before** `slice(0, limit)`, carried through `readEmailMessages()` and `buildEmailSelection()` into `email_selection` as `unreviewed_total` / `oldest_unreviewed_received_at`.
- **The new line reads:** `N email messages are unreviewed (of M eligible; K read), the oldest D days old.` — with `at least N`, `of at least M` **and `the oldest at least D days old`** when the candidate pool itself filled. **`total_is_floor` qualifies all THREE, and the age for a reason that is easy to miss:** the pool is read `.order('received_at', { ascending: false }).limit(200)`, i.e. **the 200 NEWEST eligible rows**, so when it fills, the rows that did not fit are the OLDER ones — the pool's oldest is a lower bound on the real oldest, exactly as the counts are lower bounds on the real counts. (**Corrected 2026-08-26, Law 12:** this entry first said the floor applied to the counts only, and the code matched it. A bare age understated the single number the owner is most likely to act on, while the two counts beside it hedged correctly — which is worse than either hedging or not, because the sentence looked internally consistent.) **K is the payload length, so it follows `enforceTotalSize` trimming; N and M do not** — the payload shrank, the pool did not. When no pool figure is available (a caller that supplied none, or a failed read) the old computation runs and labels its own narrower scope: `N email messages are unreviewed (of the K read), …`. **`unreviewed_total: null` means "not known" and must never be read as zero.**
- **`computeWorkspaceState`'s "cannot see a record the model was not given" property now has exactly ONE stated exception**, and its doc comment names it. Every other count there is still payload-derived. A count that describes more than the model saw is honest only if the sentence says so, which is what `(of M eligible; K read)` is for.
- **`UNREVIEWED_ACTION_STATUS` is exported by `reader.js` and imported by `compose.js`.** Both ends of one pipeline were filtering on their own `'unreviewed'` literal; a drift in either would have produced a brief whose headline number disagreed with the selection behind it, with nothing thrown and nothing logged — Lesson 20 in a second place. **This costs `compose.js` its "imports NOTHING" property**, knowingly: the header now records the one import, that it is a bare string constant, and that `client.js` does no work at import time so the module graph grows while nothing executes.
- **`alreadySentToday()` (ledger.js) is a READ-ONLY pre-check, and it is NOT a lock.** A single SELECT before the model call, on the LIVE path only, so a retry or a duplicate tick after a completed send no longer reads eight CoS tables and pays for a billable compose before the ledger says no. **`claimSend()` is unchanged and still decides the race** — between the pre-check's SELECT and the claim's INSERT another tick can claim the day, so Bundle 38's double-send test now injects `precheck: () => ({ blocked: false })` on purpose to stay inside that window and keep testing the real 23505 collision. It fails **OPEN** (`{ blocked: false, unavailable: true }`) where `claimSend` fails closed; that is safe only because `claimSend` reads the same table moments later and fails closed on the same fault. **NO insert, NO update:** a pre-check that wrote would consume the day's slot on a run that composed nothing. `dry_run` and `writeback_only` ignore it entirely — neither claims nor sends, so an existing row says nothing about whether they should run, and blocking them would make a rehearsal depend on a real send.
- **`cos.compose.ok` and a `latency_ms` on `cos.send.ok` were added the same day.** Until then the only record of a model call's cost was the `agent_runs` spend row, which is written ONLY when `COS_BRIEF_USAGE_USER_ID` is set — and it is not set on Railway, so **the job's latency and token count existed nowhere at all.**
- **`COS_USER_ID` is NOT validated at boot and is NOT in `config.js` at all.** Its only consumer is `resolveCosUserId()` in `writer.js`, which prefers it and otherwise derives the owner from the newest `today_briefs` row (announcing which path it used). `assertSecureBoot()` never mentions it, so a typo'd uuid boots clean and fails at 11:00 UTC inside the writeback. Verified 2026-08-26 by reading `src/config.js` end to end.
- **`cos.owner.derived` is CONDITIONAL. It is NOT part of every healthy run** (recorded 2026-09-04, Law 12). `resolveCosUserId()` prefers `COS_USER_ID` and derives the owner from the newest `today_briefs` row ONLY when that variable is absent. It was absent through 2026-08-26 (every dump from that day carries `cos.owner.derived` and `owner id source: derived`, between `cos.send.ok` and `cos.brief.written`) and it has been set since 2026-08-30, so the event has stopped firing: the 2026-09-04 run logs `cos.mode → cos.delivery.mode → cos.read.retried → cos.email.truncated → cos.compose.ok → cos.send.ok → cos.brief.written`, with `owner id source: settings` and no `cos.owner.derived` anywhere. **A verifier or a session that expects the event on every run is calibrated against a pre-08-30 dump.** `scripts/verify-brief-run.mjs` does not require it (its check is on relative order of the required events only), but its header comment says the event "sits between cos.send.ok and cos.brief.written on every real run in the dumps": that sentence describes the August dumps, not the present. Two things to read carefully. First, **`owner id source: settings` in every `cos.brief.written` line from 2026-08-30 to 2026-09-04 was FALSE.** Nothing in `src/` reads the `user_settings` table; the only hits are inside message strings. The job resolved the owner correctly as `env` and then threw that away: it handed only the id to `writeBriefToCos({ cosUserId })`, whose `resolveCosUserId` returned `source: 'settings'` for ANY caller-supplied id before looking at anything, inside a function whose own comment says the log must not blur levels of confidence. **Fixed 2026-09-04 (Law 12, same session):** the caller's own resolved source now travels with the id (`cosUserSource`), an id with no declared source is labelled `caller`, and `settings` can appear only when a caller genuinely reads `user_settings.cos_user_id` and says so, which nothing does until P1.4. From the next deploy the line reads `owner id source: env`. Bundle 38 proves it with a discriminating control (a caller declaring `settings` is labelled `settings`, named in the test as unexercised until P1.4) and four mutations, the first of which restores the hardcoded `'settings'`. Second, and plainly: **an environment variable is currently the thing that names the person receiving the brief.** `COS_USER_ID` decides whose CoS rows are gathered, whose ledger slot is claimed, and whose `today_briefs` row is written. That is exactly the condition P1.4 exists to remove, and it is the multi-user rule two headings above (every per-person value is a column, not a variable) stated for this one variable. **P1.4 resolves to `docs/BUILD_PLAN.md`, "Prove no environment variable names a person"**: that file is the referent for every P-number canon cites, generated from the build ledger (11 of 54 complete as of 2026-09-04). Regenerate it; never hand-edit it. (**Corrected 2026-09-10, Law 12, three ways.** (1) "Nothing in `src/` reads the `user_settings` table" was already false when written: `src/services/programs/today.js:35` read it for the program-owner lookup at `39c6d22` (`docs/SCOPE_AUDIT_2026-09-09.md` §2), and since P1.4 commit 1 (`4d93856`, merged `a81871c`) `src/services/cos/personSettings.js` reads it for the brief's own identity — so `owner id source: settings` in `cos.brief.written` is TRUE when it appears, and it can appear only when the person's row supplied the id (`docs/SESSION_N1_2026-09-09.md`). Four reading sites now; the fold below lists them. (2) `scripts/verify-brief-run.mjs`'s header no longer says the event sits on every real run — corrected on `fix/verify-brief-run-2026-09-09`, merged `bf05327` (`docs/SESSION_N5_2026-09-09.md`, owed item 2). (3) `cos.owner.derived` STILL EXISTS on `main` at `1ba6ad0`: commit 1 keeps the derive path and the env fallback behind the settings path; the withheld `28208a2` removes both.)
...
**SMS allowlists and `ALLOWED_PHONES`** (recorded 2026-08-26 — this variable appeared **zero** times in this document until now, while gating every SMS in both directions)

- **`ALLOWED_PHONES` is one variable governing two independent allowlists**, and neither is described anywhere else in canon.
- **Inbound** (Bundle 36): `src/lib/smsAllowlist.js`, enforced as **STAGE A2** in `routes/sms.js`. A number outside the list gets **empty TwiML at HTTP 200** — deliberately a well-formed no-op, not a 403: Twilio retries a non-2xx and a refusal that looks like an outage produces retry storms. The cost is that a blocked message is indistinguishable from a delivered one *at the HTTP layer*, which is why the event is logged rather than inferred from the status code.
- **Outbound** (Bundle 37): enforced at the **`sendSms()` choke point** — one function, so no caller can route around it — refusing with `OUTBOUND_REFUSED`. It is a choke point rather than a check per call site precisely because the per-call-site version is the shape that drifts.
- **It was set on Railway on 2026-08-16**, per `~/Downloads/CEDRUS_SESSION_COMPLETE_2026-08-15_to_08-17.md` §11/§13. **The value was NOT re-read tonight** — that needs `railway variables`, which was out of scope for this session. So: set on that date per the session record; current value unverified 2026-08-26.
- Live arming is **unverified**: `sms.allowlist.check` is a log-only fact and no such line appears in either dump, because no SMS traffic occurred in the window. Absence here carries no information (II.2) — the window contains no inbound messages at all.

**Production observations, 2026-08-26** (from `railway status --json` and the three log dumps in `~/Downloads`, all read-only — no `railway variables`, no `railway run`)

- **Deployed state: deployment `718c9304-9886-47cd-8741-0322f5614cbd` = commit `dee339d`, status SUCCESS, created 2026-08-26T05:49:01Z, instance RUNNING**, project `respectful-transformation`, service `cedrus-backend`, environment `production`, region `sfo`, 1 replica, healthcheck `/health`, start command `node src/index.js`. From `railway status --json`.
- **The scheduler registers ELEVEN jobs and this is now evidenced, not inferred.** The postdeploy dump's line, verbatim: `2026-08-26T05:49:29.321512510Z [INFO] jobs: reminder-dispatch, daily-sweeps, clarification-expiry, weekly-briefs, weekly-brief-emails, trial-downgrades, budget-guard, card-sender, card-followup, monthly-core-five, cos-daily-brief timestamp="2026-08-26T05:49:28.671Z" event="scheduler.started"`. Any surviving present-tense claim of 7 or 10 jobs is wrong; **dated history entries saying 7 are history and stay as written.**
- **Railway renders log levels as `[LEVEL]` TAGS, not as a field. `level=error` CAN NEVER MATCH in production, however broken the service is.** Measured across `railway_logs_2026-08-26.txt`: 1201 `[INFO]`, 2 `[WARN]`, and `grep -c 'level='` finds the string only inside message prose. **The discriminating patterns are `[ERROR]`, `[FATAL]` and bare `FATAL`.** This is a silent false negative on the first search anyone reaches for, and it is why `scripts/verify-brief-run.mjs` matches the tag. A session that greps `level=error`, finds nothing and reports "no errors" has run a check that could not have failed (Law 3).
- **Clock skew — the earlier "~29 s, Railway ahead of the CoS database" attribution is WRONG and is replaced.** That figure lives in `~/Downloads/CEDRUS_MASTER_CONTEXT.md` §7, not in this document, so there is no line here to strike — this bullet is the correction. Measured 2026-08-26: **the MacBook was ~30 s FAST** against NTP-accurate references and was re-synced that night; **Railway's HTTP `Date` header was within ~1 s of an NTP-verified reference across five samples**. The "~29 s" was manufactured by running the comparison locally against a fast laptop clock. Railway is not the clock that is off. What is real: `cos.read.retried` fires on **one random table per run, five runs out of five** (`email_ai_analyses` on 08-24, `email_messages` on 08-25 and again on 08-26), always recovering on attempt 2 — a Supabase-side transient the bounded retry absorbs. With `sb_secret_` keys the JWT behind `PGRST303` is minted at Supabase's own gateway, so it may be entirely Supabase-internal. **Keep the retry; drop the idea that Railway needs fixing.** The rule stands and gains a clause: **no row may carry a timestamp from the wrong clock — including the laptop's.** Every local run against production stamps rows and measures "skew" with whatever the laptop believes.
- **`supabase/migrations/` EXISTS in this repo and holds two tracked files** (**FOUR as of 2026-09-10 — re-corrected at the end of this bullet, Law 12**) (**corrected 2026-09-09, Law 12:** it read "exactly one" until `20260909120000_provision_user.sql` was committed that day, UNAPPLIED — see the provisioning bullet above): `20260830120000_multiuser_foundation.sql` (created 2026-08-30, Session A, applied 2026-08-31) and `20260909120000_provision_user.sql` (P1.2, not applied). `supabase/.temp/` is gitignored and there is still no `supabase/config.toml`. The `20260817000000_sources_signals.sql` the 08-17 log describes was never committed anywhere — that half was and remains true. See Law 8. (**Corrected 2026-09-03, Law 12:** this read *"`supabase/migrations/` does not exist in this repo (only a gitignored `supabase/.temp/`)"*. It was true when written on 2026-08-26 and was falsified four days later by the directory's own creation. **The document was contradicting itself in two places for four days:** Law 8's DECIDED block already said "`supabase/migrations/` now exists in this repo and holds one file", and `docs/MIGRATION_PATH.md` said the same — so a session reading II.5 for verified facts and a session reading Law 8 for the DDL path would reach opposite conclusions about whether the directory was there. Verified on disk with `git ls-files supabase/`.) (**Re-corrected 2026-09-10, Law 12:** `git ls-files supabase/` at `1ba6ad0` lists FOUR tracked migration files — the two named here plus `20260909180000_programs_foundation.sql` (R6.2a) and `20260909210000_normalize_account.sql` (P1.3), both applied to prod per their own reports. This line went stale twice in one day, which is exactly why Bundle 48 reads the directory instead of carrying a list.)
...
**Production observations, 2026-09-04 (the 1 September gap)** (read-only: `system_flags`, `agent_runs`, CoS `today_briefs` via `railway run`, the Railway deployment list, and Emil's Gmail; no row written)

- **The 2026-09-01 brief was not missing. It was delivered and then moved to Trash.** Every durable record for Sept 1 is complete and byte-shaped like its neighbours: ledger row `cos_brief_send:2026-09-01` is `status: sent` with a Resend `provider_message_id`; `agent_runs` has the `cos_daily_brief` row at 11:00:09Z, `success: true`, 6147 tokens; CoS `today_briefs` has the `status: ok` row at 11:00:10Z. Gmail holds the message (`brief@updates.cedrus.life`, received 2026-09-01T11:00:13Z, still UNREAD) under the TRASH label, while Aug 28 through Sept 4 are all in INBOX. Neither outcome (a) precheck skip, (b) job never fired, (c) stuck claim, nor (d) compose or send failure applies. **What the record cannot say is who or what moved it to Trash after delivery.** Nothing on the Cedrus side can act on a delivered message, so that is an inbox-side event (a filter, a swipe, or a deliberate delete), and it is unread, which argues against a deliberate one.
- **Sept 1, 2 and 3 all ran under the OLD global day-key** (`cos_brief_send:<date>`). The per-user key first ran on Sept 4 (`cos_brief_send:<user_id>:2026-09-04`). Railway's deployment list: `ea82371` (2026-08-26) served every 11:00Z tick until `b22d5cf` went live 2026-09-03T16:30Z, `d817172` at 21:53Z the same day, `49853d7` at 2026-09-04T11:43Z. There was no deploy, restart or build anywhere near 2026-09-01T11:00Z. The old key was the live suspect for a consumed day and it is cleared for this incident by the rows themselves.
- **The ledger's `sent_at` is the run's START time, not the transport's confirmation time.** `runCosDailyBrief` takes one `now` at entry and passes it to `markSent()`, so every row reads `11:00:01Z` while `agent_runs` and `today_briefs` for the same run are stamped 9 to 30 seconds later. Read `sent_at` as "the tick that sent it", never as send latency.
- **`RESEND_API_KEY` on Railway is a send-only restricted key.** `GET https://api.resend.com/emails/<id>` returns `401 restricted_api_key` ("This API key is restricted to only send emails") for the Sept 1 message id and for two controls. So **no session can read a message's post-handoff state (delivered, bounced, complained) from the service env.** The stored `provider_message_id` is a receipt that Resend ACCEPTED the message, nothing more. The instrument for "did it reach the inbox" is the inbox: Gmail search `in:anywhere subject:"Cedrus daily brief"`, which also reveals Trash and Spam, where a plain inbox look does not.
- **`COS_USER_ID` is set on Railway (36 chars), `COS_BRIEF_LIVE=true`, `COS_BRIEF_DRY_RUN` and `COS_BRIEF_WRITEBACK_ONLY` unset, `NODE_ENV=production`.** Observed 2026-09-04 via `railway run` (presence and length only; values not printed). The Sept 4 ledger key and every `today_briefs.user_id` since Aug 27 carry the same uuid, so the variable names the one live owner.

**The 2026-09-09 overnight sessions, folded at the 2026-09-10 morning merge ceremony** (sources: the CANON sections of `docs/SESSION_N1_2026-09-09.md`, `docs/SESSION_N2_2026-09-09.md` and `docs/SESSION_N3_2026-09-09.md`, the owed-to-the-record list in `docs/SESSION_N5_2026-09-09.md`, and `docs/SCOPE_AUDIT_2026-09-09.md` (N4); the N1 file was committed on its branch inside the WITHHELD second commit `28208a2` and is carried onto `main` by this fold so the citation resolves — the code of that commit is not merged)

- **The brief finds its person in rows** (P1.4 commit 1, `4d93856`, merged `a81871c`; Bundle 46). `selectPerson()` in `src/jobs/cosDailyBrief.js` takes the ONE `user_settings` row whose `cos_user_id` is bound. Two bound rows abort the run (`cos.brief.aborted`, reason `ambiguous_owner`) until B2.2 iterates — fail closed, never choose. Zero bound rows announce `cos.owner.settings_missing` (warn, outcome `no_row` | `null_id`, naming the person where there is one) and only then fall back to `COS_USER_ID`, labelled `env`. An unreadable table announces `cos.owner.settings_unreadable` (error, `error_code` = SQLSTATE) and is never folded into "missing" (Lesson 1). `usage_user_id` rides along from the same row into the spend record, which announces its own source (`cos.usage.recorded`, outcome `settings` | `env`). The reader is `src/services/cos/personSettings.js`: `readPersonSettings(userId)` is ONE scoped read of the Cedrus project (`lib/supabase.js`, service role — the table lives beside `app_users`, NOT in CoS), the row or `null`, a read ERROR throws, a blank id is refused; `listCosIdentityHolders()` lists the `user_id`s with a non-NULL `cos_user_id`. **No module state, no cache of any kind.** **In production today (`user_settings` empty) every run logs `settings_missing` once and then behaves exactly as before, labelled `env`** — that line is the proof that the fallback, not the row, still names Emil.
- **P1.4's second commit, `28208a2`, is on `feat/brief-from-settings-2026-09-09` and is NOT merged.** It removes the `COS_USER_ID` / `COS_BRIEF_USAGE_USER_ID` fallbacks, the derive-the-owner-from-the-newest-`today_briefs`-row bootstrap and its `Map` cache, `resetCosUserId()` and the writer's `cosSelectAcrossAllUsers` import; a missing row then aborts (`settings_missing`), an unreadable one aborts (`settings_unreadable`), nobody-named refuses (`cos.owner.unresolved`), and `cos.owner.derived` ceases to exist. **It must not merge until `user_settings.cos_user_id` is populated for every active user** (0 rows in production as of the P1.3 session's read): deployed before the backfill, every 11:00 UTC tick aborts and nobody receives a brief — loudly, but nobody. The live proof to wait for, under commit 1, at the first 11:00 UTC tick after the backfill: `owner id source: settings` in `cos.brief.written`, `usage id source: settings` in `cos.usage.recorded`, no `cos.owner.settings_missing`. After commit 2 merges, `COS_USER_ID` and `COS_BRIEF_USAGE_USER_ID` are unread by the brief and can leave Railway (Emil's action, Law 5) — except that `src/services/programs/today.js:51` still reads the usage one as the program-owner fallback (E-05 in the scope audit, N1's flag 2): the job already knows `who.personId` before it calls `programOwner`, so B2.1 should pass the person straight through and delete that fallback; the per-person rule is not closed until it goes.
- **`cos.brief.aborted` now carries `reason`** on the P1.4 sites (`settings_missing` | `settings_unreadable` | `ambiguous_owner` | `no_user_id`). Read beside N5's finding: at `39c6d22` none of the three older abort sites (unresolved owner, gather `read_failed`, program read failed) passed `reason` — they carry `error_category` and `outcome`, and the table names and the owner source live in the message prose by design, because the logger allowlist keeps free-form text out of structured fields. `scripts/verify-brief-run.mjs` prints whatever structured fields the line actually carries and never invents a `reason:`; it reads the tables from the prose in the exact shapes the job writes.
- **A sibling suite that drives your REAL entry point encodes your old mechanism** (Lesson 4 in a new place; N1). Bundle 43 (`test/programs-foundation.test.mjs`) and Bundle 48 (`test/isolation-proofs.test.mjs`) both run the real `runCosDailyBrief()` with `COS_USER_ID` and inject no settings seam, so under commit 1 every real-job run in them reaches the real Cedrus client for `user_settings`, waits ~7 s for `http://supabase.invalid` to fail, announces `settings_unreadable` and falls back to the env — green, and **the merged-main battery went from 21 s to 87 s at `a81871c`** (measured 2026-09-10: Bundle 43 alone 30 s, Bundle 48 alone 37 s; their runs capture their own console, so the lines do not appear in the battery log). Under the withheld commit 2 those runs ABORT and Bundle 43 goes RED (3 FAIL). **Owed before commit 2 merges:** the four-line `settingsDb` seam N1 wrote out for `runFor`'s deps in `docs/SESSION_N1_2026-09-09.md`, and the same seam in Bundle 48's `runJobFor`; with it the runs resolve from the row in milliseconds and each suite's `COS_USER_ID` literal becomes inert (set it to nonsense then, as Bundle 38's is). The general fact: **a read against `http://supabase.invalid` costs ~7 s** (supabase-js resolves an error, it does not throw), and any real-job run in a suite that does not inject every database seam pays it silently — which is how a green suite that takes minutes stops being run.
- **The /api choke point verifies the Supabase JWT IN CODE and requires aal2** (A3.1, Bundle 47, merged `0498880`; `docs/SESSION_N2_2026-09-09.md`). `src/services/auth/jwt.js` verifies the signature first — ES256 and RS256 against the project JWKS at `<SUPABASE_URL>/auth/v1/.well-known/jwks.json` (`keys.js`: fetched lazily, cached 10 min, one early refresh per minute on an unknown `kid`), HS256 only if `SUPABASE_JWT_SECRET` is set and never reading a JWK as a secret; then `exp`/`nbf`, `iss`, `aud`, `role = authenticated`, UUID `sub`; `alg: none` and everything else is `unsupported_alg`. Then `assessAssurance()`: `aal === 'aal2'` or **401 `mfa_required`** (`aal2_required` | `aal_missing` | `aal_unknown`). `auth.getUser` is no longer on the production path — it returns the account, not the session, and cannot see the second factor. **The dashboard TOTP toggle enables ENROLMENT only; the enforcement is in code**, applied in `createRequireUser` (`src/routes/api/auth.js`), the single choke point every `/api` router mounts. A JWKS fetch failure is `key_unavailable` → **500 at the choke point, never 401** (an outage must never read as a bad login). The verifier announces its mode on first use (`web.auth.verifier`: JWKS host, HS256 on/off, issuer, required assurance). Every refusal is a short category and no response carries the token or a segment of it.
- **Deploy consequence of A3.1 (Law 6), stated plainly for the push:** once `main` is pushed, EVERY `/api` route answers 401 `mfa_required` until Emil has enabled TOTP in Supabase (Authentication → Multi-Factor → "Enable TOTP-based MFA") and enrolled his own factor so his session is aal2. `cedrus.life` (dormant, `POST /api/capture`) signs in with password only today and will receive 401. **No secret to set:** the Cedrus project signs with ES256 and its public JWKS carried exactly one P-256 key on 2026-09-09 (`kid c3fb7d1a-6b82-45eb-bef7-6228367b37d8`, read without a credential), so verification needs only `SUPABASE_URL`, already set; set `SUPABASE_JWT_SECRET` on Railway ONLY if the project ever returns to the legacy HS256 secret.
- **An opaque (non-JWT) bearer is refused in production** (`reason: malformed`). Outside production it is delegated to `auth.getUser` — the seam exists only because seven pre-A3.1 suites fake GoTrue with opaque `tok-a` tokens — announced once per middleware instance as `web.auth.seam` (the battery log carries 15 such warn lines now). GoTrue itself parses every bearer as a JWT, so the seam can never authenticate anyone against a real project, and a real Supabase token never takes it. Migrating those suites to signed test JWTs (Bundle 47's helpers) removes the seam entirely — a follow-up, not urgent.
- **`/api/interface` resolves a person's CoS scope from THEIR `user_settings.cos_user_id` and nothing else** (A3.2; `src/services/auth/callerScope.js`, explicit `.eq('user_id', appUser.id)`; every CoS read through `forUser()`). No fallback to `COS_USER_ID`. A person with no row is `cos_not_linked`, not empty; a failed read is a **503**, never an empty 200 (`capabilities_unreadable`, `settings_unreadable`, `brief_unreadable`, `workspace_unreadable`). Until P1.3 backfills `user_settings`, Emil's own `/brief/today` and `/workspace` answer `cos_not_linked` — honestly, by design. Routes: `GET /session` (user, session `{aal, methods, issued_at, expires_at, session_id}`, the four capability booleans with an absent row = false, `preferences.skin` = a null slot because no such column exists), `GET /status` (per-service state incl. the per-user send ledger via `alreadySentToday`), `GET /brief/today` (the stored `today_brief_v1` row if its `generated_at` is today, `generation_mode='ai'` and `status='ok'`), `GET /workspace` (the reader's own projections). Shapes: `src/routes/api/interface.shapes.d.ts`, every response key pinned. Mounted in `src/index.js` BEFORE the authed `/api` catch-all, and Bundle 47 pins the order. **`user_settings` now has FOUR reading sites in `src/`**: `programs/today.js:35` (the program owner, since `39c6d22`), `cos/personSettings.js:77` and `:99` (the brief's person, P1.4), `auth/callerScope.js:40` (the caller's CoS scope, A3.2).
- **"Today" for `/brief/today` is the person's calendar day in their timezone** (`user_settings.timezone`, else `app_users.timezone`, else the default), not the server's UTC day; the send ledger stays keyed by UTC day as before.
- **Open, for the ledger (N2):** `docs/BUILD_PLAN.md` A3.2's done-when says "unauthenticated gets 404 not 401"; the session brief said 401 with a reason, and the choke point (auth before route matching under `/api`, this section's first bullet) makes 401 the only consistent answer. Built to the brief. Wanting 404 is a deliberate change to the choke point's contract, not a router tweak.
- **Charter manifest, P1.5 (Bundle 48, merged `2c16b16`; `docs/SESSION_N3_2026-09-09.md`):** A2 proven (a write against another person's record is refused — programs path through the real `publish_program_revision()`, CoS writeback path through the real job with the insert landing in a PGlite `cos.today_briefs`), A5 proven (the public key reads nothing — Cedrus database by battery, CoS by the live read below, not by battery), A10 proven (no person's identifier appears in another person's log trace, sequential AND interleaved). A9 is P1.4's. P1.5 is three of four. No `src/` file changed: all three proofs held on the code as it was.
- **Prod facts from a read-only Management-API metadata read, 2026-09-09** (`pg_class`, `pg_policies`, `pg_default_acl`, `information_schema`; no row of any table). Cedrus project `qjwbtlnwnjjuvrwblkzx`: **35 public tables, RLS ENABLED on all 35** — `app_users` and every legacy table included — **FORCED on 6** (`admin_audit`, `programs`, `program_revisions`, `program_items`, `user_capabilities`, `user_settings`). `app_users` carries two policies, `own profile read` (SELECT) and `own profile update` (UPDATE), both `to authenticated`, both `auth.uid() = auth_user_id`, and no migration in this repo declares them — Bundle 48's fixture reproduces the prod read, so if prod moves the fixture is stale. **anon holds SELECT on nothing**; exactly one policy names anon, `waitlist_signups` "anyone can join waitlist", INSERT, and anon holds INSERT on that table only. **Flag, not fixed:** the project's default ACL for tables created by `postgres` is `anon=Dxtm`, so anon holds TRUNCATE, REFERENCES, TRIGGER and MAINTAIN on most legacy tables — unreachable through the publishable key's only transport (PostgREST has no truncate verb and anon can execute no function that truncates), so not an exposure today, but a grant nobody meant; a future migration could `revoke truncate, references, trigger on all tables in schema public from anon`. 38 relations in prod carry a `*user_id*` column (6 of them views), far more than this repo's migrations declare, so Bundle 48's completeness check is about the PGlite database (this repo's DDL), not prod. CoS project `kpzyzjhfvjfvxowhusir`: **15 public tables, RLS enabled on all** (none forced), **zero policies naming anon/public, anon holds no table privilege at all** (its default ACL for `postgres` names only `postgres` and `service_role`); the eight reader tables are among the 15. **"No session here can read prod" (the bootstrap-admin bullet above) is superseded for metadata:** the Management-API SQL path that `scripts/verify-multiuser-state.py` uses is a read-only instrument available from a session, and N3 and R6.2a both used it.
- **Bundle 48 reads EVERY `<14 digits>_*.sql` in `supabase/migrations/`** (sorted) rather than listing them, enumerates the person tables from those files (every `create table if not exists`, plus every pre-existing table a migration `alter table`s) and asserts the list COMPLETE against the catalog, and reproduces Supabase's stock default privileges — anon gets ALL on a new public table until revoked — with a self-check that an unprotected throwaway table IS readable by anon, so its refusals are refusals and not a role that can read nothing. A migration that lands later joins the proof on its own: it ran 69/69 with three files and 69/69 with `main`'s fourth. Two lesson candidates for II.4: **a test of "anon reads nothing" needs BOTH a behavioural read and a structural check** — a policy that names anon but keeps the owner predicate (`user_id = current_app_user_id()`, NULL for anon) leaks nothing today and everything the day the predicate changes; and **a cross-owner write test must use CHANGED bytes** — `UNIQUE (program_id, source_sha256)` refused B's revision on A's program with 23505 even after four ownership layers were removed, an idempotency key hiding a missing owner filter (guard 3 found this fifth layer). Also: the logger lives at `src/utils/logger.js`; `src/lib/` holds cors, openai, smsAllowlist, supabase, twilio (a prompt named `src/lib/logger.js`).
- **The verifier reads the September events** (Bundle 40 EXTENDED, no new number, merged `bf05327`; `docs/SESSION_N5_2026-09-09.md`). `scripts/verify-brief-run.mjs` exports three pure readers — `abortedRun()`, `ownerSource()`, `programsBlock()` — and prints three report lines; an aborted run's abort is the FIRST line of RESULT and ends the wall-time window (a terminal line, like a writeback or a precheck skip); `HEAD`, `OPTIONAL_MIDDLE`, `LIVE_TAIL`, `SKIP_TAIL`, `checkSequence()`, `inOrder()`, `findLatestRun()` and every `return` in `report()` are byte-for-byte what they were — the sequence and the exit codes did not move (an aborted run is exit 1, which is what 1 has always meant; no fourth code). `count` and `reason` on `programs.today.read` ARE structured; `reason` there is the program block's owner source (`resolveProgramOwner`: rows first, env second), a different lookup from the written line's, and the two can disagree, so the report prints them separately. A `settings` label on the written line is repeated as printed, with the caveat that until `9e9a91d` (2026-09-04) it was stamped on any caller-supplied id. Bundle 40: 67 → 123 checks; the five new guards were mutated by hand (each RED) and their exact `mutate` triples are listed in the N5 report for `test/mutate-bundle-40.sh` — **owed, not wired**.
- **Of the seven Railway dumps under `~/Downloads`, only two exit 0 from the verifier** — `railway_logs_2026-08-26_1100run.txt` and `railway_logs_2026-09-04.txt`. `railway_logs_2026-08-26.txt` and `_filtered.txt` were exit 1 at `39c6d22` already: their newest run is 08-25's (the full dump was captured at 01:12 on 08-26), before `cos.compose.ok` shipped in `dee339d`, so to a verifier that requires that event they are broken sequences and always were. The three `*_postdeploy*` / `*_deploy.txt` files exit 2 (no run in window). **A handoff saying "the three 08-26 dumps exit 0" is miscalibrated**; Bundle 40 pins all four verdicts as unchanged, with an announced SKIP for any dump absent from the machine running it, so a silent skip cannot read as a pass.
- **The scope audit (`docs/SCOPE_AUDIT_2026-09-09.md`, at `39c6d22`) is the map for B2.x and T7.x**, the way the singleton audit was for Sessions B–E: 39 findings, 5 High — the four per-person values still in the environment (`COS_USER_ID`, `COS_BRIEF_TO`, `COS_BRIEF_USAGE_USER_ID`, `ALLOWED_PHONES`) and **T-01: no battery stage executes `src/services/cos/client.js:262`, the one line that applies `.eq('user_id', …)` on the service-role path** — Bundle 41 builds that query by hand against its own fake, so deleting the line leaves every stage green (a Law 3 gap on the isolation boundary itself; the mutation was not run because the audit session could not edit source). Its §2.1 ledger of env reads that name a person (E-01 to E-10) was cross-checked at the ceremony against what P1.4 commit 1 actually changed — recorded in II.7, 2026-09-10.
- `.gitignore` carries both `__pycache__/` (anywhere; N5) and `scripts/__pycache__/` (P1.3): a `.pyc` carries NULs and the battery's first stage fails on any tracked file that does.
...
| 17 | ~~No cost monitoring anywhere~~ **CONSUMER MERGED + SCHEMA LIVE 2026-07-29 (morning ceremony).** Budget guard merged to local `main` (`19cdf87`); `system_flags` applied to prod through the runner. Hourly job + kill-switch row + inbound/outbound gates, mutation-checked (Bundles 28/29). | **Two remaining. Corrected 2026-08-05 (Law 12):** the entry previously listed "`main` is UNPUSHED so Railway runs pre-merge code" as open. That is false — `main == origin/main @ 6723c0a`, observed 2026-08-05 — so the guard is deployed, and the claim is removed. Done: branch merged, `system_flags` migration run, code deployed. ~~Still open: `DAILY_TOKEN_BUDGET`/`DAILY_SMS_BUDGET` are NOT set on Railway (unset = DISARMED); no `budget.check` line has been observed in prod, and live-deploy confirmation against Railway logs is still owed.~~ **CLOSED 2026-08-26 (Law 12) — observed in the logs, not inferred.** `~/Downloads/railway_logs_2026-08-26.txt` carries **49** `budget.check` lines spanning 2026-08-24T04:10:01Z → 2026-08-26T05:10:01Z, and **every one of them** reads `mode=armed tokens=…/750000 sms_segments=…/400 persisted=true … outcome="ok" reason="under_budget"` — zero lines not `mode=armed` (`grep 'event="budget.check"' … | grep -cv 'mode=armed'` → 0). That is simultaneously the live-deploy confirmation that was owed: the guard is running hourly in the deployed process. Re-confirmed after tonight's deploy — 11:10Z and 12:10Z both read `tokens=6167/750000`, counting the 11:00 brief's own spend, so the model-calling job IS logging its run and the guard is not undercounting. **STILL OPEN, and it is the only part left: `quota.read.failed` has no alert consumer.** Verified in code 2026-08-26 — it is emitted at `services/budget.js:31` and `services/usage.js:28`, asserted in Bundles 21/28, and read back by nothing: no Sentry, no PagerDuty, no Opsgenie, no webhook, no log drain. The two source comments calling it "alertable" are aspirational, not wiring. The guard will enforce; it pages nobody. |
...
**Register audited 2026-08-26.** Every flag marked open was checked against the code (grep) and the three log dumps in `~/Downloads`. **One flag moved: 17, whose DISARMED half is closed on 49 `budget.check` lines.** Nothing else had evidence sufficient to strike it, and nothing was struck for tidiness — the master file's warning that this register "only ever grows" is fair, but the fix is evidence, not pruning. Flags **5, 6, 7, 8, 9, 12 (frontend half), 13, 14, 18, 19, 20, 22** were re-checked and each is **still open, verified 2026-08-26** (14's number was re-counted: 68 of 114). Flags **2** and **9** are **unverifiable from this session** and are marked so rather than guessed: 2 needs the Railway Activity feed, 9 is not currently reproducible. The full table with the evidence for each sits in `~/Downloads/OVERNIGHT_REPORT_2026-08-26.md` §4.

**One thing the audit established that no flag had claimed:** across all three dumps — 1393 lines spanning 2026-08-24T03:43Z to 2026-08-26T13:00Z — there are **ZERO** `job.failed`, `*.failed`, `*.error` or `*.aborted` events of any kind (`grep -hoE 'event="[a-z.]*(failed|error|aborted|stuck|unresolved|unrecorded)[a-z.]*"'` → no matches), **zero `[ERROR]` and zero `[FATAL]` lines**, and `environment="production"` on all 1203 lines of the main dump. The 08-26 live-state verification listed "the other ten jobs complete their ticks without `job.failed`" as unverified; it is now verified for this window. **Read it with the window in mind** — absence over 57 hours is evidence about 57 hours, not a property of the system (II.2).
...
- **2026-09-10 (THE BATTERY REPORTING RULE — settled once, at the morning merge ceremony)** — Two sessions on 2026-09-09 reported "59 suites" and "37 stage banners" for the same battery and could not reproduce each other, because neither had written its method down. **From now on a battery is reported with exactly these numbers, produced by exactly these commands, from the saved output of one run:**
```sh
sh test/run-all.sh > "$LOG" 2>&1; echo "exit=$?"   # THE GATE: the exit code, nothing else (Law 4)
grep -c -E '^=== .* ===$' "$LOG"                   # suites-run  = stage banners the battery printed
grep -c '^  PASS' "$LOG"                           # strict PASS = assertion lines
grep -c '^  FAIL' "$LOG"                           # strict FAIL = must be 0
```
  **suites-run** is the number of stage banners the battery prints: every line of the form `=== … ===` in the combined stdout+stderr of `sh test/run-all.sh`. That is the banners `run-all.sh` echoes itself (28 at `a129d66`) PLUS the banners its sub-rigs and some suites print for themselves (the NUL scan, the three N2 email rigs, and Bundles 41–44 each print one of their own), so it is larger than the count of `echo "=== ` lines in the script and it moves whenever a stage is added. It counts a stage that RAN — passed, failed, or skipped with an announcement (the CoS schema check without credentials, the contracts stage without a toolchain, both print their banner either way) — so it proves the battery reached that stage and nothing about coverage. **strict PASS** is the count of assertion lines beginning with two spaces and `PASS`; **strict FAIL** the same for `FAIL`, and a report may say `FAIL=0` only when that count is 0 AND the exit code is 0. A green report reads `exit 0, FAIL=0, suites-run N, strict PASS M`; the exit code is the gate and the other three exist for reconciliation — a strict PASS delta between two runs must reconcile to the suites that changed and nothing else (the way 2732 = 2668 + Bundle 44's 64). Measured on `a129d66` (P1.3 merged, before this ceremony's first merge): **exit 0, FAIL=0, suites-run 37, strict PASS 2732** — the numbers the P1.3 entry below recorded, reproduced by this method. **The "59 suites" figure does not reproduce from that log by any banner grep** — `^ALL .*PASSED` gives 58, adding the `BUNDLE n: ALL PASSED` lines gives 61, the bare `PASSED` substring 67, unique test files referenced by the six rigs 89, suites invoked directly by `run-all.sh` 27 — and its method was never recorded, so it is retired as a battery number: where an older report says "59 suites", read it as an unreproducible count from a green run, never as a target. Never count `PASSED` banners, never count `PASS` as a substring (the R6.2a entry below records what that did to the P1.2 handoff), and never gate on the halfway `ALL WS-B SUITES PASSED` line (II.5, Config).
- **2026-09-10 (morning merge ceremony — six overnight branches, one at a time, `--no-ff`, battery on merged `main` after every code branch)** — Start: `main` at `a129d66`, clean, 3 ahead of `origin/main` (`39c6d22`); every branch was cut from `39c6d22`, so each conflicted additively with P1.3's Bundle 44 stage in `test/run-all.sh` — resolved by keeping both sides, ascending (44, 46, 47, 48), never dropping a line. First the reporting rule above (`ac77fa7`). Then, with the battery by that rule after each: **(1) `2c16b16`** `test/isolation-proofs-2026-09-09` (Bundle 48) — exit 0, FAIL=0, suites-run 39, strict PASS 2801 = 2732 + 69, exactly N3's prediction; **(2) `a81871c`** `feat/brief-from-settings-2026-09-09` merged UP TO `4d93856` ONLY (commit 1 of 2: settings first, env fallback kept and announced; **`28208a2` withheld until Emil confirms `user_settings.cos_user_id` is populated**) — exit 0, FAIL=0, suites-run 40, strict PASS 2855 = 2801 + 34 (Bundle 46) + 20 (Bundle 38's new P1.4 section); battery 21 s → 87 s, the settings-path timeouts in Bundles 43 and 48 (II.5); **(3) `0498880`** `feat/auth-api-2026-09-09` (Bundle 47) — exit 0, FAIL=0, suites-run 42, strict PASS 2991 = 2855 + 136; **(4) `bf05327`** `fix/verify-brief-run-2026-09-09` (Bundle 40 extended, merged clean incl. `.gitignore`) — exit 0, FAIL=0, suites-run 42, strict PASS 3047 = 2991 + 56, N5's predicted delta; **(5) `acc36ab`** `docs/scope-audit-2026-09-09` and **(6) `1ba6ad0`** `docs/executor-design-2026-09-09`, docs only — no battery, the NUL scan (`sh test/no-nul-bytes.sh`, the battery's first stage, run after `git add`) green after each. `interface/v10` left alone. **One battery went red and it was not a merge:** the first run at `2c16b16` died at the CORS stage (stage 7, before the new stage) with `ECONNREFUSED` after 12 s — `test/cors.test.mjs` does `app.listen(0)` and fires its first request at once, a startup race; the suite is byte-identical to the green baseline, the branch changed no `src/` file, it passed 3/3 alone in 0.2 s, and git greps had been running concurrently. Re-run on a quiet machine: green. Recorded here so the next session that sees it does not spend an hour on it, and does not treat "flaky" as an excuse either — the re-run, not the reasoning, is the gate. **The canon fold** (this commit): the CANON sections of N1, N2, N3 and N5's owed list into II.5, with the Law 12 corrections they forced (the bundle register: 46, 47, 48 claimed, 40 extended, next free 49; four migration files, not two; `user_settings` IS read, four sites; the one cross-user read named correctly per the scope audit's F1-05; the `resolveCosUserId` cache bullet marked superseded; the verifier-header clause struck). **The cross-check, N4 §2.1 against what N1 actually changed, verified by grep on `1ba6ad0`:** N4 lists ten env reads that name a person or a per-person setting (E-01 to E-10). N1's merged commit touched TWO — E-01 `COS_USER_ID` (`writer.js`, now behind the settings path, still read as the fallback) and E-03 `COS_BRIEF_USAGE_USER_ID` (`cosDailyBrief.js`, now behind `usage_user_id`, still read as the fallback) — and its withheld commit removes exactly those two reads. **Eight reads N4 found that N1 did not touch and that are still read on `main`:** E-02 `COS_BRIEF_TO` (`resendTransport.js:60`, `config.js:114`); E-04 `ALLOWED_PHONES` (`config.js:65`); E-05 `COS_BRIEF_USAGE_USER_ID` as the program owner (`programs/today.js:51` — N1 flagged it, Law 9); E-06 the brief hour constant (`scheduler.js:52`); E-07 `TESTER_PHONES` (`config.js:57`); E-08 `ADMIN_EMAIL` and the admin secrets (`adminAuth.js:203–205, 210`); E-09 the hard-coded reply-to (`resendTransport.js:51`, `COS_BRIEF_REPLY_TO` at `:62`); E-10 `DEFAULT_TIMEZONE` (`config.js:37`). Bundle 47 added two env reads, both secrets/globals (`SUPABASE_URL`, `SUPABASE_JWT_SECRET` in `auth/verifier.js`), and none that names a person. That is why N4 ran independently: the list is the B2.x/T7.x backlog, and P1.4 closes two of ten when its second commit lands. **Nothing pushed, nothing applied, no backfill run** (Laws 5, 8). Next, in this order and by Emil: `python3 scripts/apply-normalize-account-via-api.py`; the backfill dry run `python3 scripts/backfill-accounts-via-api.py --cos-user-id <COS_USER_ID> --usage-user-id <COS_BRIEF_USAGE_USER_ID>` (both from `railway variables`), read the plan, then again with `--commit`; then `git push origin main` — knowing the push also deploys A3.1, so every `/api` route answers 401 `mfa_required` until TOTP is enabled and enrolled (II.5).
- **2026-09-09 (P1.3 — backfill through the provisioning path)** — Branch `feat/backfill-accounts-2026-09-09`. `provision_user()`'s tail (settings defaults, capability grants, one audit row) extracted into `normalize_account(actor, user_id, capabilities)` — idempotent, grants brought to EXACTLY the given set with revocations kept as `granted = false`, one audit row per call — and `provision_user` redefined to call it, so a fresh account and a backfilled one share ONE code path (the migration asserts it from `pg_proc`). `bind_cos_identity(actor, user_id, cos_user_id, usage_user_id)`: the audited hand-off of `COS_USER_ID` / `COS_BRIEF_USAGE_USER_ID` from Railway to `user_settings`; refuses an un-normalised account and a CoS id already bound elsewhere. A fresh account now carries TWO audit rows (`provision_user`, `normalize_account`); Bundle 42 updated to apply the P1.3 file and say so. `scripts/backfill-accounts-via-api.py`, dry-run by default, takes the two ids as arguments and never reads Railway. **Not applied, not pushed, backfill not run.** Merged `--no-ff` into local `main` as `133c6d9`; battery on merged main exit 0, FAIL=0, 37 stage banners (30 after the WS-B banner), strict PASS 2732 = 2668 + 64. Report: `docs/BACKFILL_ACCOUNTS_2026-09-09.md`. Proof: Bundle 44 (64 checks) with the done-when as a diff plus controls, and `test/mutate-bundle-44.sh` (21 guards, 0 missed) including the two named mutations in layers: skipping the settings insert goes red on the function's post-condition and then on the migration's diff guard; an inline tail in `provision_user` with a deliberate timezone difference goes red on the one-code-path assert, then the migration's diff, then the suite's diff; an IDENTICAL inline tail is invisible to every diff and caught only by the structural pin. One live-behaviour note for the day the backfill runs: `programs/today.js` already resolves the program owner rows-first from `user_settings.cos_user_id`, so Emil's row will name him the moment it is bound (same person, no deploy).
- **2026-09-09 (R6.2a — programs data foundation)** — Branch `feat/programs-foundation-2026-09-09`. Three additive tables (`programs`, `program_revisions`, `program_items`) to the interface's approved contract, RLS enabled and forced, `user_id` on every table with composite owner FKs; `publish_program_revision()` and `todays_program_items()`, both SECURITY DEFINER and `service_role`-only; a dry-run-by-default load script; the brief's computed today's-program block, rendered first. **Not applied, not pushed, no source loaded.** Report: `docs/PROGRAMS_2026-09-09.md`. Two Law 12 corrections in II.5 from a prod read the same evening: P1.2's migration IS applied, and `user_settings` is empty. **Battery reporting rule from this session's task 0:** a "PASS count" is method-dependent — `grep -c PASS` counts the sixty-odd `PASSED` banner lines and two `BYPASSED` section headers, the strict `^  PASS` count does not — and the 2603-vs-2578 discrepancy in the P1.2 handoff was exactly that (2603 = substring count on 9e9a91d; 2578 = strict count on 6fecb69; strict-on-both is 2537 vs 2578, the difference being Bundle 42's 41 checks and nothing else). Reports cite suites run and FAIL=0, gated on `echo $?`, not a PASS total.
- **2026-09-04 (the owner-source label)** — `cos.brief.written` had claimed `owner id source: settings` on every run since 2026-08-30 while nothing in `src/` reads `user_settings`: `writer.js` labelled any caller-supplied id `settings` before looking at anything, and the job, which had resolved `env` correctly, passed only the id down. Fixed: the job's own resolved source travels with the id, an undeclared caller id is labelled `caller`, and `settings` is reserved for a caller that genuinely read the table (none exists until P1.4). No `user_settings` read was invented to make the label true. Bundle 38 gained the discriminating control and four mutations (restoring the hardcoded `'settings'` goes red). `docs/BUILD_PLAN.md` committed as the referent for the P-numbers this document cites.
- **2026-09-04 (the 1 September gap, read-only)** — Traced the missing Sept 1 brief through the durable record instead of the expired logs: ledger, `agent_runs`, CoS `today_briefs`, the Railway deployment list and Gmail all agree the run completed and the email was delivered at 11:00:13Z; it was later moved to Trash on the inbox side and is still unread. No code or data changed. Recorded in II.5: the finding; that Sept 1 to 3 ran under the old global day-key with no deploy near the tick; that ledger `sent_at` is the tick time; that the Resend key is send-only so post-handoff delivery state is unreadable from a session; and that `cos.owner.derived` is conditional on `COS_USER_ID` being absent, has not fired since that variable was set on 2026-08-30, and that `owner id source: settings` is the writeback's label for a caller-supplied id, not evidence that `user_settings` was read. Plainly stated in the same bullet: an environment variable currently names the person who receives the brief, which P1.4 exists to remove.
- **2026-08-30 (Session A — multi-user foundation)** — Cedrus became allowlist-only multi-user. Branch `feat/multiuser-foundation-2026-08-30`. **The singleton audit is the session's most valuable output** (`docs/SINGLETON_AUDIT_2026-08-30.md`): 19 findings, each with file:line, produced by grepping `src/` rather than from memory, and it is the map for Sessions B–E. Fixed here: the per-user ledger key, the `forUser()` scoping wrapper (with `cosSelect` REMOVED from the export surface), the `resolveCosUserId` process-global cache, the `account_status` and capability guards in `createRequireUser` / `requireCapability`. Prepared not applied: `supabase/migrations/20260830120000_multiuser_foundation.sql` (`app_users` role/status/invitation columns, `user_capabilities`, `user_settings`, `admin_audit`, RLS forced on all three, no `anon` policy anywhere) plus `docs/MIGRATION_PATH.md` with the six repair commands. **Bundle 41** (`test/multiuser-isolation.test.mjs`, 59 PASS) with `test/mutate-bundle-41.sh` — 9 guards, 0 missed. Battery **2466 → 2525 PASS, 0 FAIL** at every stage. Law 8's DDL decision recorded as MADE (Supabase CLI). **`trap restore EXIT` added to mutate-bundle-38, -40 and -41**, and its necessity proven by reproducing the 2026-08-26 incident: an interrupted un-trapped run left `scripts/verify-brief-run.mjs` mutated on disk, and the identical interrupt against the trapped version restored it byte-identically. Corrected: the master context's §2 and §10 (which would have made a future session refuse this work), the stale "43 guards" figure (already stale at `ea82371`; it is 48), and the reader's own single-owner justification comment. **NOTHING PUSHED, no migration applied, no database write** (Laws 5 and 8).
...
- **Tests:** `sh test/run-all.sh`. **Gate on `echo $?`, never on the banner.** `ALL WS-B SUITES PASSED` prints roughly HALFWAY through with many suites still to run (re-measured 2026-08-18: line 1803 of 3588, 19 suites after it). The exact position moves every time a bundle is added — never memorise it, and see II.5. Assert the count of suite banners appearing after it.
- **Rigs:** there are **three** concat rigs. Grep all three whenever imports change.
- **Migrations:** **the sanctioned DDL path is the Supabase CLI** — one file per migration in `supabase/migrations/`, applied with `supabase migration up --linked`, never a bare `db push` (Law 8, **decided 2026-08-30**, superseding the 2026-08-26 "pending" note). `~/.config/cedrus/migrate/run-migration.mjs` still does not exist and was never in version control. `supabase/migrations/` **now exists** and holds `20260830120000_multiuser_foundation.sql`. Six file-less remote versions need `supabase migration repair --status applied <version>` before the first apply; see `docs/MIGRATION_PATH.md`. Still: in order, before the code that needs them (Law 11), and **only Emil applies** (Law 5). **The `supabase` CLI is NOT installed on this machine** (`which supabase` → not found), so no session here can run or rehearse a migration — it can only prepare the file.
- **Untouchable:** `safetyDetection.js`, `safetyFlags.js`, `voiceGuard.js`.
- **Open:** `BRIEF_DRY_RUN` stays `true` until a named arming session. The two admin router mounts remain unproven. `briefs.sent_at` is still written under dry run (its nullability could not be verified without a migration-level schema read, so it was left alone — see the 2026-08-18 session notes). (**Corrected 2026-08-17, Law 12:** this line said `DAILY_TOKEN_BUDGET` and `DAILY_SMS_BUDGET` were "unset on Railway, so the budget guard is deployed but disarmed." Both are set — 750000 and 400 — and the guard is armed and enforcing. Verified live 2026-08-18 02:39Z. See II.5.)

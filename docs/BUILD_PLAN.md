...
**11 of 57 complete as of 2026-09-09.**
...
2026-09-09: amended for the Cedrus Consulting direction (`cedrus-consulting.md`):
C4.1 is the V10 handoff via Codex, A3.3 takes its input from Codex's adapter
shapes, R6.2a pulls the programs data foundation forward to sit after P1.2,
Phase 8 doubles as install #1, and Phase 9 (skills) is added, unscheduled.
The order of Phases 0–3 did not change.
...
- [ ] **R6.2a — Programs data foundation — pulled forward, data only**
  - Phase 6's programs work, brought forward as the data layer only because the triathlon plan is both Emil's real use and the sales demo. Three additive tables (`programs`, `program_revisions` with `source_text`, `program_items`), RLS forced and `user_id` on every one, a dry-run-by-default load script, and a computed today's-program block in the brief. Two programs on day one: Miami Man 2026 (training) and the care routine. No rendering until Phase 4; no SMS from items. One migration at a time, so this waits for P1.2 to land and apply.
  - *Owner:* Claude Code · *Proofs:* —
  - *Done when:* Publishing the same source twice creates one revision, not two — with the control that a changed source creates a second. Cross-user read returns zero of the other person's items, with the owner's read as control, in the same test.
...
- [ ] **A3.3 — Freeze the shapes in a contract**
  - A JSON Schema plus example fixtures generated by running the real handlers, and a battery stage that fails when they drift. **Input:** the adapter shapes Codex reports as TypeScript at the end of Prompt B (C4.1). They flow one way — Codex reports them, this item freezes them.
  - *Owner:* Claude Code · *Proofs:* —
...
- [ ] **C4.1 — The V10 handoff: inventory, then keep/rebuild/delete**
  - Now the rebrand. Codex inventories V9 — every surface, token, fixture and on-screen number — and writes the keep/rebuild/delete proposal as `web/V10_PROPOSAL.md` (Prompt A, no code). Emil approves or amends it. Then Codex builds on branch `interface/v10` (Prompt B): two skins on one component tree, one data adapter, a demo badge on every non-derived value, mobile-first, PWA-installable. Codex works only in `web/`, never `src/`, never pushes. Candidates already named — keep the visual system, the layer model, the Runs grammar; rebuild the hard-coded day, the invented drift inbox, the decorative numbers.
  - *Owner:* Codex · *Proofs:* —
  - *Done when:* Prompt A's proposal approved by Emil before Prompt B writes a line; Prompt B ends with 24 screenshots (six surfaces × two skins × 390px and 1280px) and the adapter shapes as TypeScript.
...
- [ ] **R6.2 — Programs, rendered and lived in**
  - The data foundation landed early as R6.2a. What remains: the Programs surface renders from `program_items` (a ten-week grid, categories per day, the growth mascot as state), sessions with planned, proposed and actual kept separate, and citations in the brief.
  - *Owner:* Claude Code · *Proofs:* —
  - *Done when:* A real program renders end to end from the three tables, on the phone, with nothing hard-coded.
...
## Phase 8 — The Mac Studio — and install #1

Local inference and agent execution on hardware you own, reachable from anywhere over the tailnet, never from the public internet. This is also install #1 of Cedrus Second Brain, timed: hours logged, and every judgment call a stranger's business would not permit written down.
...
- [ ] **M8.2 — Install and benchmark the local model**
  - A 70B at 8-bit fits comfortably in 128 GB. Measure tokens per second and time to first token before depending on it. This benchmark is what answers the quality-versus-privacy question — hybrid by default or fully local — with measured numbers instead of expected ones.
  - *Owner:* Emil · *Proofs:* —
...
## Phase 9 — Skills — the asset with no home

Skills are the business's biggest asset and have no home in the Engine: there is no concept in code for how one is packaged, loaded, versioned, or shared across accounts. Unscheduled, after Phase 6. Do not pull it forward.

- [ ] **S9.1 — Define what a skill is, in code**
  - A skill is a packaged capability the Engine loads — turn a transcript into a dated plan, run a ten-week block, draft from house templates. Decide the package shape, how one is loaded and versioned, and how the Engine knows which are present.
  - *Owner:* Claude Code · *Proofs:* —
  - *Done when:* One real skill packaged, loaded and versioned through the mechanism; a second version supersedes the first without the first disappearing.
- [ ] **S9.2 — Share a skill across accounts without sharing data**
  - The same skill runs for Emil, Dad and a client install. Capability is portable; the data it touches is not.
  - *Owner:* Claude Code · *Proofs:* —
  - *Done when:* One skill, two accounts, each sees only its own data — with the control that each does see its own.

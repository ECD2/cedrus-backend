...
import { findContactDisclosure } from '../guards/fabrication.ts';

export const OPERATOR_REVIEW_VERSION = 1;

export const REVIEW_DECISIONS = ['approved', 'edited', 'killed'] as const;
...
export const operatorReviewValidator = inspect(reviewShape, {
  ...
  run: (review, path): readonly Issue[] => {
    const issues: Issue[] = [];
      ...
      if (review.reason_code === null) {
        issues.push(
          issue(
            ...
            `a ${review.decision} decision must carry a reason code; that log is the training signal`,
          ),
        );
      }
      if (review.reason_text === null) {
        issues.push(
          issue(`${path}reason_text`, 'review/missing_reason', `a ${review.decision} decision must carry a written reason`),
        );
      }
    }
    ...
    if (review.decision === 'edited' && review.edited_statement_refs.length === 0) {
      issues.push(
        issue(
          `${path}edited_statement_refs`,
          'review/edit_without_change',
          'an edited decision must name the statements that changed',
        ),
      );
    }
    if (review.decision !== 'edited' && review.edited_statement_refs.length > 0) {
      issues.push(
        issue(
          `${path}edited_statement_refs`,
          'review/change_without_edit',
          'only an edited decision carries changed statements',
        ),
      );
    }
...
export const operatorReviewContract: Contract<OperatorReview> = defineContract(
  {
    name: 'cedrus.operator_review',
    version: OPERATOR_REVIEW_VERSION,
    title: 'Operator review',
    description:
      'The review decision on one card. Edits and kills require a reason, because that log is what says which cards to automate first.',

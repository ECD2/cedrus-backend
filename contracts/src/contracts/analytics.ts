...
import { CARD_OUTCOMES } from './card-outcome.ts';
import { REVIEW_DECISIONS, REVIEW_REASON_CODES } from './operator-review.ts';
import { ASSISTANT_JOBS } from '../common/primitives.ts';
import { walk } from '../schema/core.ts';
...
const FORBIDDEN_SET: ReadonlySet<string> = new Set(FORBIDDEN_ANALYTICS_FIELDS.map(normaliseKey));
...
const envelope = <const N extends AnalyticsEventName, P extends Record<string, Validator<unknown>>>(
  name: N,
  props: P,
) =>
  object({
...
const noProps = {} as Record<string, Validator<unknown>>;
...
export const analyticsEventValidators = {
  ...
  'onboarding.step_completed': envelope('onboarding.step_completed', {
    step: enumOf(['name', 'work_setup', 'neighborhood', 'free_windows', 'activities', 'current_groups', 'people', 'social_prefs'] as const),
  }),
  ...
  'context.supplied': envelope('context.supplied', {
    kind: enumOf(['stated_windows', 'activity', 'person', 'place_preference'] as const),
  ...
  'connection.revoked': envelope('connection.revoked', {
    connection_ref: id('The connection.'),
    initiated_by: enumOf(['member', 'provider', 'operator'] as const),
  }),
  ...
  'card.delivered': envelope('card.delivered', {
    card_ref: id('The card.'),
    channel: enumOf(['web', 'sms'] as const),
  }),
  'card.outcome': envelope('card.outcome', {
    card_ref: id('The card.'),
    outcome: enumOf(CARD_OUTCOMES),
    source: enumOf(['tap', 'sms', 'no_response', 'unknown'] as const),
  }),
  ...
  'return.visit': envelope('return.visit', {
    days_since_last: enumOf(['7_to_13', '14_to_29', '30_plus'] as const),
  }),
  ...
  'payment.intent_expressed': envelope('payment.intent_expressed', {
    surface: enumOf(['sms', 'web', 'in_person'] as const),
  }),
} as const;
...
const unionMembers = ANALYTICS_EVENT_NAMES.map((name) => ({
  tag: name,
  validator: analyticsEventValidators[name] as unknown as Validator<AnalyticsEvent>,
}));

const baseUnion = discriminatedUnion<'event', AnalyticsEvent>('event', unionMembers);

/** Vanity names get their own rejection, so the error explains the doctrine. */
const vanityAware: Validator<AnalyticsEvent> = {
  kind: 'analytics-union',
  schema: baseUnion.schema,
  fullyExpressedInJsonSchema: baseUnion.fullyExpressedInJsonSchema,
  check(input, path) {
    if (typeof input === 'object' && input !== null && !Array.isArray(input)) {
      const name = (input as Record<string, unknown>)['event'];
      if (typeof name === 'string' && (VANITY_EVENT_NAMES as readonly string[]).includes(name)) {
        return [
          issue(
            path === '' ? 'event' : `${path}.event`,
            'analytics/vanity_metric',
            `"${name}" is a vanity metric; Cedrus instruments the loop, not the pageviews`,
          ),
        ];
      }
    }
...
export const analyticsEventValidator = inspect(vanityAware, {
  ...
  run: (event, path): readonly Issue[] => {
    const issues: Issue[] = [];
    walk(event, path, (node) => {
      if (node.key === null) return;
      const normalised = normaliseKey(node.key);
      if (!FORBIDDEN_SET.has(normalised)) return;
      if (event.event === TEXT_BEARING_EVENT && node.key === TEXT_BEARING_FIELD) return;
      issues.push(
        issue(
          node.path,
          'analytics/content_in_event',
          `"${node.key}" carries message or calendar content; analytics carry ids and enums`,
        ),
      );
    });
    return issues;
...
export const analyticsEventContract: Contract<AnalyticsEvent> = defineContract(
  {
    name: 'cedrus.analytics_event',
    version: ANALYTICS_VERSION,
    title: 'Analytics event',
    description:
      'One instrumented event from the loop. Ids and enums only, with one named exception for the out-of-scope request log.',
...
export const isVanityEventName = (name: string): boolean =>
  (VANITY_EVENT_NAMES as readonly string[]).includes(name);

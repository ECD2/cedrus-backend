...
import { assistantJob, id, instant, memberId } from '../common/primitives.ts';

export const AGENT_REQUEST_VERSION = 1;

export const REQUEST_CHANNELS = ['sms', 'web'] as const;

export const RESPONSE_KINDS = ['answered', 'honest_decline', 'safety_fixed_register', 'deferred_to_operator'] as const;
 ...
 */
export const FIXED_REGISTER_TRIGGERS = ['stop', 'help', 'distress', 'none'] as const;
...
export const agentRequestValidator = inspect(requestShape, {
  ...
  run: (request, path): readonly Issue[] => {
    const issues: Issue[] = [];

    if (request.in_scope) {
      if (request.scope_job === null) {
        issues.push(
          issue(`${path}scope_job`, 'agent_request/in_scope_without_job', 'an in-scope request must name which job it is'),
        );
      }
    } else {
      if (request.scope_job !== null) {
        issues.push(
          issue(
            `${path}scope_job`,
            'agent_request/out_of_scope_with_job',
            'an out-of-scope request may not be filed under a job it does not belong to',
          ),
        );
      }
      if (!request.logged_as_request) {
        issues.push(
          issue(
            `${path}logged_as_request`,
            'agent_request/out_of_scope_not_logged',
            'an out-of-scope request must be logged; that log is the roadmap',
          ),
        );
      }
      if (request.response_kind === 'answered') {
        issues.push(
          issue(
            `${path}response_kind`,
            'agent_request/out_of_scope_answered_as_in_scope',
            'anything outside the narrow promise gets an honest answer, not a confident one',
          ),
        );
      }
    }
    ...
    if (request.fixed_register_trigger !== 'none') {
      if (request.voice_applied) {
        issues.push(
          issue(
            `${path}voice_applied`,
            'agent_request/tone_shifted_safety_reply',
            'STOP, HELP and distress replies stay in a fixed register regardless of the voice preference',
          ),
        );
      }
      if (request.response_kind !== 'safety_fixed_register') {
        issues.push(
          issue(
            `${path}response_kind`,
            'agent_request/safety_trigger_wrong_response',
            'a fixed-register trigger must be answered in the fixed register',
          ),
        );
      }
    }
...
export const agentRequestContract: Contract<AgentRequest> = defineContract(
  {
    name: 'cedrus.agent_request',
    version: AGENT_REQUEST_VERSION,
    title: 'Agent request',
    description:
      'One inbound request. Open input, narrow promise: out of scope is recorded as out of scope, logged, and answered honestly.',
 ...
 */
export const assistantPromiseValidator = refine(
  object({

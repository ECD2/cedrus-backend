...
import { countValidator, id, instant, memberId } from '../common/primitives.ts';

export const ENVELOPE_VERSION = 1;

export const ENVELOPE_KINDS = ['export', 'import'] as const;
...
const FORBIDDEN_SET: ReadonlySet<string> = new Set(FORBIDDEN_ENVELOPE_FIELDS);
...
const envelopeShape = object({
  ...
  source_system: enumOf(['cedrus_backend', 'cedrus_miami', 'operator_tool'] as const),
  ...
  redactions: arrayOf(
    object({
      contract: enumOf(PORTABLE_CONTRACTS),
      reason: enumOf(['credentials_never_exported', 'retained_legal_record', 'not_member_data'] as const),
...
export const dataEnvelopeValidator = inspect(envelopeShape, {
  ...
  run: (envelopeValue, path): readonly Issue[] => {
    const issues: Issue[] = [];
    ...
    if (claimed.value !== claimed.source_refs.length) {
      issues.push(
        issue(
          `${path}integrity.record_count`,
          'fabrication/count_not_derived',
          `record_count says ${claimed.value} but cites ${claimed.source_refs.length} record(s)`,
        ),
      );
    }
    if (claimed.value !== envelopeValue.records.length) {
      issues.push(
        issue(
          `${path}integrity.record_count`,
          'envelope/count_mismatch',
          `record_count says ${claimed.value} but the envelope carries ${envelopeValue.records.length}`,
        ),
      );
    }
    ...
    for (let i = 0; i < claimed.source_refs.length; i += 1) {
      ...
      if (!carried.has(ref)) {
        issues.push(
          issue(
            `${path}integrity.record_count.source_refs[${i}]`,
            'envelope/count_cites_missing_record',
            `record_count cites "${ref}", which the envelope does not carry`,
          ),
        );
      }
    }
    ...
    walk(envelopeValue, path, (node) => {
      ...
      if (FORBIDDEN_SET.has(normaliseKey(node.key))) {
        issues.push(
          issue(
            node.path,
            'envelope/credential_leak',
            `"${node.key}" is a credential; tokens are server-side only and never travel in an envelope`,
          ),
        );
      }
    });
...
export const dataEnvelopeContract: Contract<DataEnvelope> = defineContract(
  {
    name: 'cedrus.data_envelope',
    version: ENVELOPE_VERSION,
    title: 'Import/export envelope',
    description:
      'A versioned batch of records for export or import. Counts name what they counted, redactions are disclosed, and credentials never travel.',

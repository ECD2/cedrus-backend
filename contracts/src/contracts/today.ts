...
import { id, instant, localDate, memberId, phoneDigits } from '../common/primitives.ts';
import { paceCardValidator } from './pace-card.ts';
import { statementValidator } from './statement.ts';

export const TODAY_VERSION = 1;

export const TIMING_BASES = ['stated', 'calendar'] as const;
...
const todayShape = object({
  ...
  fallback_notice: nullable(
    enumOf(['using_your_usual_windows', 'calendar_disconnected', 'calendar_stale'] as const),
  ),
  ...
  run: (today, path): readonly Issue[] => {
    ...
    if (today.timing_basis === 'stated' && today.day_line.kind !== 'inferred') {
      issues.push(
        issue(
          `${path}day_line.kind`,
          'today/stated_timing_presented_as_known',
          `timing derived from stated windows is an inference; day_line.kind was "${today.day_line.kind}"`,
        ),
      );
    }
    if (today.timing_basis === 'calendar' && today.day_line.kind === 'user_reported') {
      issues.push(
        issue(
          `${path}day_line.kind`,
          'today/calendar_timing_mislabelled',
          'calendar-derived timing is known or inferred, never user_reported',
        ),
      );
    }
    ...
    if (today.day_line.kind === 'known' && today.day_line.source.type !== 'calendar_freebusy') {
      issues.push(
        issue(
          `${path}day_line.source.type`,
          'today/known_day_line_without_calendar',
          'a known statement about today\'s timing must come from the free/busy projection',
        ),
      );
    }
    ...
    if (needsNotice && today.fallback_notice === null) {
      issues.push(
        issue(
          `${path}fallback_notice`,
          'today/silent_fallback',
          'Today fell back to stated windows and must say so; a silent fallback looks certain while guessing',
        ),
      );
    }
    if (!needsNotice && today.fallback_notice !== null) {
      issues.push(
        issue(`${path}fallback_notice`, 'today/notice_without_fallback', 'a fallback notice without a fallback is misleading'),
      );
    }
    ...
    if (today.card !== null) {
      ...
      if (today.card.status !== 'delivered') {
        issues.push(
          issue(
            `${path}card.status`,
            'today/undelivered_card_shown',
            'Today shows delivered cards only; a draft has not been reviewed',
          ),
        );
      }
    }
    ...
    if (today.card !== null && today.empty_state !== null) {
      issues.push(
        issue(`${path}empty_state`, 'today/empty_state_with_card', 'an empty state cannot accompany a card'),
      );
    }
...
export const todayStateContract: Contract<TodayState> = defineContract(
  {
    name: 'cedrus.today_state',
    version: TODAY_VERSION,
    title: 'Today state',
    description:
      'The state of the day and the one next thing. Timing derived from stated windows is always labelled an inference, and a fallback always says so.',

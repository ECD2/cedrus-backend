...
import { id, instant, localDate, memberId } from '../common/primitives.ts';
import { checkNoLaundering } from '../guards/provenance.ts';
import { statementValidator, type Statement } from './statement.ts';

export const PACE_CARD_VERSION = 1;

/** Reboot plan §19 `pace_cards.status`. `delivered` is the only terminal state. */
export const PACE_CARD_STATUSES = ['draft', 'approved', 'edited', 'killed', 'delivered'] as const;
export type PaceCardStatus = (typeof PACE_CARD_STATUSES)[number];

export const PACE_CARD_CHANNELS = ['web', 'sms'] as const;
...
const cardShape = object({
  ...
  window: refine(
    object({ starts_at: instant('Window start.'), ends_at: instant('Window end.') }),
    {
 ...
 */
const toProvenanceNode = (
  statement: Statement,
): { readonly kind: Statement['kind']; readonly statement_id: string; readonly derived_from?: readonly string[] | undefined } => {
  switch (statement.kind) {
    ...
    default:
      return { kind: (statement as Statement).kind, statement_id: (statement as Statement).statement_id };
  }
};

/** Card-level rules that no single field can express. */
const cardRules = (card: Infer<typeof cardShape>, path: string): readonly Issue[] => {
  const issues: Issue[] = [];
  const parts: readonly Statement[] = card.parts;
  ...
  if (proposals.length !== 1) {
    issues.push(
      issue(
        `${path}parts`,
        'pace_card/not_one_action',
        `a card proposes exactly one action, found ${proposals.length}; two cards means neither is the one thing`,
      ),
    );
  }
  ...
  if (grounded.length === 0) {
    issues.push(
      issue(
        `${path}parts`,
        'pace_card/no_grounded_statement',
        'a card must rest on at least one known or user-reported statement; all-inference is a guess with a layout',
      ),
    );
  }
  ...
  for (let i = 0; i < parts.length; i += 1) {
    const part = parts[i];
    if (part === undefined || part.kind !== 'proposed_action') continue;
    if (part.goal_ref !== card.goal_ref) {
      issues.push(
        issue(
          `${path}parts[${i}].goal_ref`,
          'pace_card/action_goal_mismatch',
          'the proposed action must serve the goal the card is hung on',
        ),
      );
    }
  }
  ...
  if (card.status === 'delivered') {
    if (card.review_ref === null) {
      issues.push(
        issue(
          `${path}review_ref`,
          'pace_card/delivered_unreviewed',
          'a delivered card must cite the operator review that approved it',
        ),
      );
    }
    if (card.delivered_at === null || card.delivered_via === null) {
      issues.push(
        issue(`${path}delivered_at`, 'pace_card/delivery_not_recorded', 'a delivered card records when and where it went'),
      );
    }
  } else if (card.delivered_at !== null) {
    issues.push(
      issue(`${path}delivered_at`, 'pace_card/delivery_without_status', 'only a delivered card carries a delivery time'),
    );
  }
...
export const paceCardContract: Contract<PaceCard> = defineContract(
  {
    name: 'cedrus.pace_card',
    version: PACE_CARD_VERSION,
    title: 'Pace card',
    description:
      'One card: a goal, tagged statements about what is known and what is inferred, and exactly one proposed action sized to a window.',

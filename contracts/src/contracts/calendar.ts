...
import { findCalendarContent } from '../guards/calendar-boundary.ts';
 ...
 */
export const busyIntervalValidator = refine(
  object({
 ...
 */
export const PROJECTION_FRESHNESS = ['live', 'stale', 'disconnected'] as const;

export const calendarFreeBusyProjectionValidator = inspect(
  object({
...
export const calendarFreeBusyProjectionContract: Contract<CalendarFreeBusyProjection> = defineContract(
  {
    name: 'cedrus.calendar_freebusy_projection',
    version: CALENDAR_PROJECTION_VERSION,
    title: 'Calendar free/busy projection',
    description:
      'Busy intervals for a bounded forward window. Titles, descriptions, locations and attendees have no representation in this contract.',
 ...
 * looking certain."
 */
export const AVAILABILITY_BASES = ['stated', 'calendar'] as const;
export const AVAILABILITY_VERSION = 1;

export const availabilityValidator = refine(
  object({
    ...
    open_windows: arrayOf(
      refine(
        object({ starts_at: instant('Open from.'), ends_at: instant('Open until.') }),
        {
          code: 'window/ends_before_starts',
          message: 'ends_at must be after starts_at',
          expressedInJsonSchema: false,
  ...
  }),
  {
    ...
    message:
      'availability based on stated windows must carry a fallback notice, and calendar-based availability must not',
    expressedInJsonSchema: false,
...
export const availabilityContract: Contract<Availability> = defineContract(
  {
    name: 'cedrus.availability',
    version: AVAILABILITY_VERSION,
    title: 'Availability',
    description:
      'Open windows for one day, labelled with whether they came from stated usual windows or from a live calendar projection.',

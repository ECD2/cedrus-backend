...
import { instant, memberId } from '../common/primitives.ts';

export const VOICE_PREFERENCE_VERSION = 1;

export const TONE_PRESETS = ['casual', 'direct', 'warm'] as const;
 ...
 */
export const TONE_APPLIES_TO = ['assistant_replies', 'pace_cards', 'nudges'] as const;

const appliesTo = () =>
  arrayOf(enumOf(TONE_APPLIES_TO), {
    minItems: 1,
...
export const voicePreferenceValidator = discriminatedUnion<'mode', VoicePreference>('mode', [
  { tag: 'preset', validator: presetVoicePreferenceValidator as Validator<VoicePreference> },
  { tag: 'free_form', validator: freeFormVoicePreferenceValidator as Validator<VoicePreference> },
]);

export const voicePreferenceContract: Contract<VoicePreference> = defineContract(
  {
    name: 'cedrus.voice_preference',
    version: VOICE_PREFERENCE_VERSION,
    title: 'Voice preference',
    description:
      'How Cedrus talks to a member. Presets now, free form as the destination. Safety and compliance registers are outside its reach.',

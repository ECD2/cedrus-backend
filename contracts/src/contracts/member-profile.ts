...
import { WORK_SETUPS, id, instant, memberId, neighborhood, phoneDigits, timezone, weekday, localTime } from '../common/primitives.ts';
 ...
 * a specific day.
 */
export const statedWindowValidator = refine(
  object({
    weekday: weekday(),
...
export const memberProfileContract: Contract<MemberProfile> = defineContract(
  {
    name: 'cedrus.member_profile',
    version: MEMBER_PROFILE_VERSION,
    title: 'Member profile',
    description:
      'The fast profile. Only name and phone are required; everything else is skippable and absent means not asked.',

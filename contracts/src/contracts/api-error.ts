...
const FORBIDDEN_SET: ReadonlySet<string> = new Set(FORBIDDEN_ERROR_FIELDS.map(normaliseKey));
...
export const apiErrorValidator = inspect(apiErrorShape, {
  ...
  run: (error, path): readonly Issue[] => {
    const issues: Issue[] = [];

    if (error.code === 'contract_violation') {
      if (error.contract === null) {
        issues.push(
          issue(`${path}contract`, 'api_error/violation_without_contract', 'a contract violation names the contract it violated'),
        );
      }
      if (error.issues.length === 0) {
        issues.push(
          issue(`${path}issues`, 'api_error/violation_without_issues', 'a contract violation lists what failed'),
        );
      }
    }
    ...
    if (error.code === 'internal_error' && error.debug_ref === null) {
      issues.push(
        issue(
          `${path}debug_ref`,
          'api_error/opaque_internal_error',
          'an internal error must carry a debug reference; opaque errors cost hours',
        ),
      );
    }
    ...
    if (error.retryable && error.retry_after_seconds === null) {
      issues.push(
        issue(`${path}retry_after_seconds`, 'api_error/retryable_without_delay', 'a retryable error says when to retry'),
      );
    }
    if (!error.retryable && error.retry_after_seconds !== null) {
      issues.push(
        issue(`${path}retry_after_seconds`, 'api_error/delay_without_retryable', 'a non-retryable error has no retry delay'),
      );
    }
    ...
    walk(error, path, (node) => {
      ...
      if (FORBIDDEN_SET.has(normaliseKey(node.key))) {
        issues.push(
          issue(
            node.path,
            'api_error/content_in_error',
            `"${node.key}" would put member or calendar content in an error body; an error body is a log line`,
          ),
        );
      }
    });
...
export const apiErrorContract: Contract<ApiError> = defineContract(
  {
    name: 'cedrus.api_error',
    version: API_ERROR_VERSION,
    title: 'API error',
    description:
      'A failure response. Never a 2xx, always specific enough to act on, never carrying member or calendar content.',

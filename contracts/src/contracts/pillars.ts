...
import { countValidator, goalLane, id, instant, localTime, memberId, neighborhood, weekday } from '../common/primitives.ts';
import { PERSON_ORIGINS, checkCountIsDerived, findContactDisclosure, findFabricatedProgress } from '../guards/fabrication.ts';
...
export const PLACE_SUITABILITY = ['calls', 'deep_work', 'either'] as const;
...
export const placeContract: Contract<Place> = defineContract(
  {
    name: 'cedrus.place',
 ...
 */
export const placeSuggestionValidator = refine(
  object({
    schema_version: literal(PLACE_VERSION),
...
export const placeSuggestionContract: Contract<PlaceSuggestion> = defineContract(
  {
 ...
 * introduction.
 */
export const INTRODUCTION_STATES = ['none', 'requested', 'both_opted_in', 'declined'] as const;
...
export const personValidator = inspect(personShape, {
  expressedInJsonSchema: false,
  run: (person, path): readonly Issue[] => {
    const issues: Issue[] = [];
    if (person.introduction_state === 'both_opted_in' && person.introduction_consent_refs.length !== 2) {
      issues.push(
        issue(
          `${path}introduction_consent_refs`,
          'people/introduction_not_double_opt_in',
          'an introduction requires a consent event from each side, for this introduction',
        ),
      );
    }
    if (person.introduction_state !== 'both_opted_in' && person.introduction_consent_refs.length > 0) {
      issues.push(
        issue(
          `${path}introduction_consent_refs`,
          'people/consent_without_introduction',
          'consent refs may only exist on a completed double opt-in',
        ),
      );
    }
...
export const personContract: Contract<Person> = defineContract(
  {
    name: 'cedrus.person',
    version: PERSON_VERSION,
    title: 'Person',
    description:
      "Someone the member told Cedrus about. Never invented, never enriched, never carrying contact details.",
...
export const PLAN_KINDS = ['suggestion', 'prepared', 'help_scheduling'] as const;

export const planValidator = refine(
  object({
...
export const planContract: Contract<Plan> = defineContract(
  {
    name: 'cedrus.plan',
...
export const progressionValidator = inspect(progressionShape, {
  expressedInJsonSchema: false,
  run: (progression, path): readonly Issue[] => {
    const issues: Issue[] = [];
    ...
    for (let i = 0; i < progression.lines.length; i += 1) {
      ...
      if (line.confirmed_helped.value > line.outcomes_recorded.value) {
        issues.push(
          issue(
            `${base}.confirmed_helped`,
            'fabrication/progress_exceeds_evidence',
            'more confirmed-helped than recorded outcomes; progress may not exceed its evidence',
          ),
        );
      }
    }
    ...
    if (progression.nothing_moved && totalOutcomes > 0) {
      issues.push(
        issue(`${path}nothing_moved`, 'fabrication/progress_contradicts_counts', 'nothing_moved contradicts recorded outcomes'),
      );
    }
    if (!progression.nothing_moved && totalOutcomes === 0) {
      issues.push(
        issue(
          `${path}nothing_moved`,
          'fabrication/progress_contradicts_counts',
          'no outcomes were recorded, so the honest answer is nothing_moved',
        ),
      );
    }
...
export const progressionContract: Contract<Progression> = defineContract(
  {
    name: 'cedrus.progression',
    version: PROGRESSION_VERSION,
    title: 'Progression',
    description:
      'Per goal: suggestions, outcomes and confirmed help, each counted from cited records. No score, no streak, and allowed to say nothing moved.',

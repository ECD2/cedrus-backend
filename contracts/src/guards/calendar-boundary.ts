 ...
 */
const PROVIDER_PREFIXES = ['event', 'calendar', 'cal', 'gcal', 'meeting', 'appointment', 'item'] as const;

const normaliseKey = (key: string): string => key.toLowerCase().replace(/[^a-z0-9]/g, '');

const FORBIDDEN_SET: ReadonlySet<string> = new Set(FORBIDDEN_CALENDAR_FIELDS);
...
const isForbiddenKey = (key: string): boolean => {
  ...
  for (const prefix of PROVIDER_PREFIXES) {
    if (normalised.startsWith(prefix) && normalised.length > prefix.length) {
      if (FORBIDDEN_SET.has(normalised.slice(prefix.length))) return true;
    }
  }
  return false;
};
 ...
 */
export const findCalendarContent = (payload: unknown, path = ''): readonly Issue[] => {
  const issues: Issue[] = [];
    ...
    if (isForbiddenKey(node.key)) {
      issues.push(
        issue(
          ...
          `"${node.key}" is calendar content; Cedrus reads busy intervals only (reboot plan §17)`,
        ),
      );
    }
  });
...
export const assertNoCalendarContent = (payload: unknown, path = ''): void => {
  const issues = findCalendarContent(payload, path);
  if (issues.length > 0) throw new CalendarBoundaryViolation(issues);
};
 ...
 */
export const forbiddenCalendarPropertyNamesSchema = (): { [key: string]: unknown } => ({
  propertyNames: {
    not: {

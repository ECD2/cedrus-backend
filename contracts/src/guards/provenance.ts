...
import { issue, matchesAnyWord, notAnyOfPatterns, type Issue, type JsonSchema } from '../schema/core.ts';

export const STATEMENT_KINDS = ['known', 'user_reported', 'inferred', 'proposed_action'] as const;
...
export const KNOWN_SOURCE_TYPES = ['calendar_freebusy', 'member_stated', 'operator_entered', 'system_record'] as const;

/** Source types that can back a `user_reported` statement. */
export const USER_REPORTED_SOURCE_TYPES = ['member_sms', 'member_tap', 'operator_entered'] as const;

/** Source types that can back an `inferred` statement. */
export const INFERRED_SOURCE_TYPES = ['model_inference', 'heuristic', 'statistical_prior'] as const;

export interface StatementLike {
  readonly kind: StatementKind;
  readonly text: string;
  readonly source?: { readonly type: string; readonly ref: string } | undefined;
  readonly derived_from?: readonly string[] | undefined;
}

/** Check 1. The cited source type must be verifiable for a `known` statement. */
export const checkKnownSource = (sourceType: string, path: string): readonly Issue[] => {
  if ((KNOWN_SOURCE_TYPES as readonly string[]).includes(sourceType)) return [];
  return [
    issue(
      path,
      'provenance/inference_as_known',
      `a known statement may not cite "${sourceType}"; verifiable sources are: ${KNOWN_SOURCE_TYPES.join(', ')}`,
    ),
  ];
};

/** Check 2a. A `known` statement may not hedge. */
export const checkKnownLanguage = (text: string, path: string): readonly Issue[] => {
  const hit = matchesAnyWord(text, HEDGE_MARKERS);
  if (hit === null) return [];
  return [
    issue(
      path,
      'provenance/hedged_known',
      `a known statement is phrased as a guess ("${hit}"); label it inferred or state it without the hedge`,
    ),
  ];
};

/** Check 2b. An `inferred` statement may not be phrased with certainty. */
export const checkInferredLanguage = (text: string, path: string): readonly Issue[] => {
  const hit = matchesAnyWord(text, CERTAINTY_MARKERS);
  if (hit === null) return [];
  return [
    issue(
      path,
      'provenance/certain_inference',
      `an inferred statement is phrased as fact ("${hit}"); hedge it or promote it to known with a verifiable source`,
    ),
  ];
};
 ...
 */
export const checkNoLaundering = (
  statements: readonly { readonly kind: StatementKind; readonly statement_id: string; readonly derived_from?: readonly string[] | undefined }[],
  path: string,
): readonly Issue[] => {
  const kindById = new Map<string, StatementKind>();
  for (const s of statements) kindById.set(s.statement_id, s.kind);

  const issues: Issue[] = [];
  for (let i = 0; i < statements.length; i += 1) {
    const s = statements[i];
    if (s === undefined || s.kind !== 'known') continue;
      ...
      if (parentKind === 'inferred' || parentKind === 'user_reported') {
        issues.push(
          issue(
            ...
            `a known statement may not be derived from a ${parentKind} statement (${ref})`,
          ),
        );
      }
    }
...
export const noHedgeSchema = (): JsonSchema => notAnyOfPatterns(HEDGE_MARKERS);

/** JSON Schema fragment forbidding certainty language in a text field. */
export const noCertaintySchema = (): JsonSchema => notAnyOfPatterns(CERTAINTY_MARKERS);

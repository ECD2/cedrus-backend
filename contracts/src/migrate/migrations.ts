...
import { MigrationRegistry, cannotFabricate, type MigrationStep, type StepResult } from './registry.ts';
import { GOAL_VERSION } from '../contracts/goals.ts';
import { MEMBER_PROFILE_VERSION } from '../contracts/member-profile.ts';
import { CARD_OUTCOME_VERSION } from '../contracts/card-outcome.ts';
import { CONNECTION_VERSION } from '../contracts/connection.ts';
...
const notAnObject = (contract: string): StepResult => ({
  ok: false,
 ...
 */
export const goalV1ToV2: MigrationStep = {
  ...
  up(input) {
    ...
    if (record === null) return notAnObject('cedrus.goal');
    return {
      ok: true,
      value: {
        ...record,
 ...
 *     value is present.
 */
export const memberProfileV1ToV2: MigrationStep = {
  ...
  up(input) {
    const record = asRecord(input);
    if (record === null) return notAnObject('cedrus.member_profile');

    const openToIntroductions = record['open_to_introductions'];
    if (typeof openToIntroductions !== 'boolean') {
      return cannotFabricate(
        'open_to_introductions',
        'recommendable',
        'the member controls whether they can be recommended (CEDRUS.md I.7 item 7); ask, do not default',
      );
    }
    ...
    if (Array.isArray(interests)) next['activities'] = interests;
 ...
 * one forever after.
 */
export const cardOutcomeV1ToV2: MigrationStep = {
  ...
  up(input) {
    const record = asRecord(input);
    if (record === null) return notAnObject('cedrus.card_outcome');
 ...
 */
export const connectionV1ToV2: MigrationStep = {
  ...
  up(input) {
    const record = asRecord(input);
    if (record === null) return notAnObject('cedrus.connection_authorization');
    ...
    if (purpose === undefined || namedOutcome === undefined || disclosure === undefined) {
      return cannotFabricate(
        'named_outcome',
        'named_outcome, purpose and disclosure',
        'these record what the member was shown and agreed to; re-authorize under the new pre-consent screen instead',
      );
    }
...
export const migrationPlanSummary = (): readonly {
  ...
  [goalV1ToV2, memberProfileV1ToV2, cardOutcomeV1ToV2, connectionV1ToV2].map((step) => ({

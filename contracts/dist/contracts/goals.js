 ...
 */
import { arrayOf, boolean, defineContract, enumOf, inspect, integer, literal, nullable, object, optional, string, } from "../schema/core.js";
 ...
 */
export const GOAL_ORIGINS = ['user_set', 'cedrus_inferred', 'operator_entered'];
/**
 * AMENDED AT VENDOR TIME (2026-08-05). Catalog item 2.
 *
 ...
 */
export const GOAL_STATUSES = ['open', 'completed', 'missed', 'canceled'];
 ...
 */
export const goalSetValidator = inspect(object({
    ...
    run: (set, path) => {
        ...
        for (let i = 0; i < set.goals.length; i += 1) {
            ...
            if (goal.member_id !== set.member_id) {
                issues.push(issue(`${path}goals[${i}].member_id`, 'goal_set/member_mismatch', 'a goal set may only hold one member\'s goals'));
            }
...
export const goalSetContract = defineContract({

 ...
 */
import { boolean, defineContract, enumOf, literal, nullable, object, optional, string, } from "../schema/core.js";
...
export const CONSENT_CHANNELS = ['email_marketing', 'email_transactional', 'sms_assistant', 'connection'];
...
export const consentEventContract = defineContract({
    name: 'cedrus.consent_event',
    version: CONSENT_VERSION,
    title: 'Consent event',
    description: 'One consent decision, per purpose, with the exact wording. Append-only: superseded, never rewritten.',
...
export const permissionStateValidator = object({
    ...
    suppressed: boolean(),
    suppression_reason: nullable(enumOf(['hard_bounce', 'complaint', 'manual'])),
...
export const permissionStateContract = defineContract({
    name: 'cedrus.permission_state',
    version: 1,
    title: 'Permission state',
    description: 'Current send permission, computed from consent events. Every broadcast filters against this, never against what a provider last knew.',

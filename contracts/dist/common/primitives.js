 ...
 */
import { arrayOf, enumOf, integer, literal, object, optional, refine, string, } from "../schema/core.js";
// --- identifiers -----------------------------------------------------------
...
export const id = (description) => string({ pattern: ID_PATTERN, minLength: 5, maxLength: 96, description });
export const memberId = () => id('Canonical member id, `app_users` is canonical (reboot plan §15).');
...
export const instant = (description) => string({ pattern: ISO_INSTANT_PATTERN, description, format: 'date-time' });
export const DATE_PATTERN = '^\\d{4}-\\d{2}-\\d{2}$';
export const localDate = (description) => string({ pattern: DATE_PATTERN, description, format: 'date' });
export const LOCAL_TIME_PATTERN = '^([01]\\d|2[0-3]):[0-5]\\d$';
export const localTime = (description) => string({ pattern: LOCAL_TIME_PATTERN, description });
/**
...
export const phoneDigits = () => string({ pattern: PHONE_DIGITS_PATTERN, description: 'Digits only, no plus. Matches app_users.phone.' });
export const TIMEZONE_PATTERN = '^[A-Za-z]+/[A-Za-z_+-]+$';
export const timezone = () => string({ pattern: TIMEZONE_PATTERN, description: 'IANA timezone.' });
// --- closed vocabularies ---------------------------------------------------
export const GOAL_LANES = ['work', 'people', 'body'];
export const goalLane = () => enumOf(GOAL_LANES);
...
export const neighborhood = () => enumOf(NEIGHBORHOODS);
export const WORK_SETUPS = ['fully_remote', 'hybrid', 'flexible_other'];
export const WEEKDAYS = ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'];
export const weekday = () => enumOf(WEEKDAYS);
/**
...
export const assistantJob = () => enumOf(ASSISTANT_JOBS);
// --- counted values --------------------------------------------------------
/**
 ...
 */
export const COUNT_BASES = ['observed_rows', 'operator_verified'];
...
export const SELF_REPORT_SOURCE_TYPES = ['member_sms', 'member_tap', 'operator_entered'];
export const INFERENCE_SOURCE_TYPES = ['model_inference', 'heuristic', 'statistical_prior'];
export const ALL_SOURCE_TYPES = [
    ...VERIFIABLE_SOURCE_TYPES,
    ...SELF_REPORT_SOURCE_TYPES.filter((s) => !VERIFIABLE_SOURCE_TYPES.includes(s)),
    ...INFERENCE_SOURCE_TYPES,
];
export const sourceRef = (allowed) => object({
    type: enumOf(allowed),
    ref: id('The record this claim was read from.'),
});
// --- time windows ----------------------------------------------------------
export const windowValidator = refine(object({
...
export const durationMinutes = () => integer({ minimum: 5, maximum: 720, description: 'Window size in minutes.' });
 ...
 */
export const schemaVersionField = (version) => literal(version);
export const optionalNote = () => optional(string({ maxLength: 500, description: 'One line of member free text.' }));

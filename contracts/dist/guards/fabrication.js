 ...
 */
export const checkCountIsDerived = (count, path) => {
    if (count.value !== count.source_refs.length) {
        return [
            issue(path, 'fabrication/count_not_derived', `count says ${count.value} but cites ${count.source_refs.length} record(s); a count must name what it counted`),
        ];
    }
...
const PROGRESS_SET = new Set([...FORBIDDEN_PROGRESS_FIELDS, ...FORBIDDEN_ENGAGEMENT_FIELDS]);
/** Check 2. No score, streak, or engagement metric anywhere in the payload. */
export const findFabricatedProgress = (payload, path = '') => {
    const issues = [];
    walk(payload, path, (node) => {
        ...
        if (PROGRESS_SET.has(normalised)) {
            issues.push(issue(node.path, 'fabrication/derived_score', `"${node.key}" is a derived score or engagement metric; Progression reads outcomes directly (reboot plan §19)`));
        }
    });
    return issues;
};
 ...
 */
export const PERSON_ORIGINS = ['member_stated', 'operator_entered'];
export const checkPersonOrigin = (origin, path) => {
    if (PERSON_ORIGINS.includes(origin))
        return [];
    return [
        issue(path, 'fabrication/invented_person', `a person may not originate from "${origin}"; allowed origins are: ${PERSON_ORIGINS.join(', ')}`),
    ];
};
/**
...
const CONTACT_SET = new Set(FORBIDDEN_CONTACT_FIELDS);
export const findContactDisclosure = (payload, path = '') => {
    const issues = [];
    walk(payload, path, (node) => {
        if (node.key === null)
            return;
        if (CONTACT_SET.has(normaliseKey(node.key))) {
            issues.push(issue(node.path, 'fabrication/contact_disclosure', `"${node.key}" would disclose contact details; introductions are double opt-in and per person (CEDRUS.md I.7.5)`));
        }
    });
    return issues;
};
export const forbiddenProgressPropertyNamesSchema = () => ({
    propertyNames: {
